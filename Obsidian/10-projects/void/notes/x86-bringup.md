...
- **Веха 87, higher-half.** Трамплин 32→64 адресует символы АБСОЛЮТНО, поэтому после переезда
  ядра наверх каждая ссылка до перехода в верхнюю половину пишется как `sym - KVA` (включая базу
  в `gdt64_ptr` и адрес входа в PVH-ноте — поле 32-битное). `retf` тоже 32-битный: прыгаем в
  НИЗКИЙ 64-битный огрызок, а он уходит наверх через `movabs` + косвенный `jmp`. Окон адресации
  два (RAM `0xFFFF_8000_…`, образ `0xFFFF_FFFF_8000_…` — модель кода `kernel`), таблицы трамплина
  строят оба плюс identity.
- **GRUB грузит multiboot2-ELF по `p_vaddr`, а не `p_paddr`** — на пути QEMU (PVH) этого не видно.
  После переезда наверх ядро уезжало в никуда: «Booting \`VOID'» → мгновенный ребут. Лечение —
  теги MB2-заголовка: адресов (type 2 — сырая заливка файла по физике + обнуление `.bss`) и входа
  (type 3). Подробности — [[higher-half]].
- **VGA-буфер `0xB8000` — физика.** Пока стояло identity-окно, всё работало; после `mm_enable`
  первый же вывод падал, а обработчик #PF снова писал в VGA → triple fault и тихий ребут.
