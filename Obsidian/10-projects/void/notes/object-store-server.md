...
- **Нет `SET_ROOT`/именованных корней по IPC**: положенный объект не привязан к корню, поэтому
  GC его соберёт — он не переживает перезагрузку. Персистентность самого пространства (Веха 7.2)
  не затронута; выдать право на корень как capability — следующий логичный кусочек.
