...
- ~~x86: операционная RAM зажата 256 МиБ~~ → **гигабайты есть (Веха 85, [[platform]])**: объём
  ...
  Осталось: потолки несимметричны — **riscv 2 ГиБ, x86 1 ГиБ** (user-регион прибит линкером к
  [1 ГиБ, 2 ГиБ), а direct-map тождественный). **План: Веха 87** — регион процессов уезжает на
  128 ГиБ, direct-map гигастраницами, потолки снимаются на обеих арх ([[0010-address-space-layout]];
  higher-half отложен сознательно).
- Аллокатор фреймов **не уважает карту памяти** (bump по ОДНОМУ диапазону поверх низкой RAM; всё,
  что выше PCI-дыры 3–4 ГиБ, не используется вовсе). **План: Веха 88** — карта как список регионов.
