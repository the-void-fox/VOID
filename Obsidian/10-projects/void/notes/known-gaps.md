...
Честная инвентаризация «недоделанного» на конец **Вехи 85** (папки, редактор строки, освобождение фреймов, AHCI-диск, установка на диск, e1000-сеть, USB-клавиатура, userspace-драйверы, хостинг Linux-драйверов до `ping`, фаза vvsh и гигабайты RAM взяты).
...
- Только **ARP + IPv4 + ICMP echo** (`ping`) в `net-srv`. **Нет UDP, TCP, DHCP, DNS** — адрес
  и шлюз зашиты константами под QEMU-SLIRP, программа не может открыть соединение.
...
## Устойчивость ядра и ресурсы (аудит перед Фазой 6, 2026-07)

Отдельный раздел: это не «фич не хватает», а **вещи, которые сегодня работают только потому, что
нагрузки нет**. С сетью и пакетами каждая превращается в невоспроизводимое падение. План — Веха 89.

- **Замки не IRQ-safe.** `sync.rs` честно оговаривает допущение: «спин-лок НЕ выключает прерывания,
  это безопасно, пока обработчики не берут те же замки». С Вехи 9 допущение НАРУШЕНО: `virtio_blk::
  on_irq` (контекст прерывания) будит future, `wake_by_ref` берёт `EXECUTOR.lock()`, а `push_back`
  в очередь готовых может дёрнуть `ALLOCATOR.lock()`. Прерывание в момент удержания того же замка
  обычным кодом = дедлок на одноядерной машине. Точечные заплатки уже есть (`irq_save_disable`
  вокруг `ASYNC` в `ReadFuture::poll`; флаг вместо замка в userdrv-IRQ — там в комментарии прямо
  написано «иначе дедлок»), но системного решения нет.
- **Store не проверяет то, на чём стоит весь тезис.** `Store::ensure_loaded` читает кадр по
  (сектор, длина) и декодирует **без сверки BLAKE3 с ContentId**; `read_object`/`write_object`
  игнорируют возвращаемый `bool` у `BlockIo`. Итог: повреждённый сектор отдаётся как валидный
  объект, неудачная запись теряется молча. Пока store наполняется только своими семенами — не
  видно; как только туда поедут скачанные пакеты — станет источником «необъяснимых» поломок.
  (Возможно, сюда же относится наблюдавшийся ранее эффект «индекс показывает 0 объектов» после
  чередования крупных put и загрузок.)
- **Ёмкость носителя нигде не учитывается.** `trait BlockIo` не имеет `capacity()`, `next_free`
  растёт без верхней границы — конец диска обнаружится только по молчаливо провалившейся записи.
  **ПОДТВЕРЖДЕНО НА ПРАКТИКЕ (Веха 87):** рабочий `void-disk.img` (16 МиБ = 32768 секторов) дорос
  до `next_free = 33923` — записи за краем пропали, суперблок указал на несуществующий индекс-кадр,
  и мост стал показывать «поколение 228 · **объектов 0**». Ядро в текущей сессии продолжало
  работать (объекты живут в RAM-кэше), потеря вылезала только после ребута — отсюда мистический
  вид симптома, который раньше списывали на «латентный баг индекса». Диагностика на секунду:
  `next_free × 512` против размера образа. Обход — образ с запасом (сейчас 64 МиБ);
  лечение — Веха 89.
- **Нет политики нехватки ресурсов.** `frame::alloc().expect(...)` в ~10 местах (таблицы страниц,
  стек процесса, кольца virtio) — исчерпание RAM роняет ЯДРО, а не просителя. Квот на процесс нет:
  `SYS_MAP` двигает `heap_brk` без лимита, страницы приходят по фолту, пока не кончится память.
- **Слоты процессов не переиспользуются.** `Table::procs` только растёт (`push` в двух местах),
  `State::Finished` остаются навсегда, `next_runnable` обходит их линейно. Долгая сессия с сотнями
  `exec` (= обычная работа пакетного менеджера) пухнет и замедляется.
- **У ядерных задач нет guard page.** Стек — `vec![0u8; 32 КиБ]` из кучи; переполнение молча
  затирает соседние данные вместо честного фолта.
...
- **Аллокатор vvsh — bump с пустым `dealloc`.** 4-МиБ арена на весь сеанс, память не возвращается
  никогда; основной шелл системы обязан жить долго, а он «протекает» на каждой команде.
