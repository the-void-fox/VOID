...
Взято Вехами 90–92: ~~свои 200 строк ARP/ICMP~~ → **vendored smoltcp**; ~~опрос~~ → **сон до
...
- **Нет TCP** (Веха 93) — программа по-прежнему не может открыть соединение; собраны только
  UDP/ICMP/DHCP/DNS.
- Нет **сокет-API**: `std::net` в порте не реализован, единственный интерфейс — IPC к `net-srv`.
...
- `net-srv` обслуживает **один запрос за раз**: на время `ping`/`resolve` он крутит стек в
  блокирующем цикле, остальные клиенты ждут. Для TCP это придётся переделать в состояние на сокет.
