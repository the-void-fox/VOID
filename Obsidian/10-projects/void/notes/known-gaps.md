...
- **HTTPS нет** (Веха 95) — `https://` честно отвергается; без него `cache.nixos.org` недоступен.
- HTTP-клиент умеет только `GET`, не держит соединение (`Connection: close`), и файл ограничен
