...
- ~~На riscv энтропия не криптографическая~~ → **virtio-rng** (перед Вехой 95): драйвер
  ...
  **Открыто для реального железа:** virtio-rng есть только под гипервизором, `RDRAND` — с Ivy
  Bridge (2012); на ASUS X54C (Sandy Bridge) сильного источника не будет, и **строить на нём
  ключи TLS нельзя**. Нужен либо сбор джиттера уровня Linux (измеримая оценка энтропии), либо
  внешний источник.
