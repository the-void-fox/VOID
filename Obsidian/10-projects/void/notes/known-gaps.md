...
3. **Освобождение фреймов** — техдолг, иначе течёт. ← следующее
4. Крупное: драйверы реального железа (AHCI/NVMe/xHCI+USB/NIC/UEFI-GOP) — покрывают ~80%
   PC своими силами; длинный хвост (wifi/GPU) — Genode-стиль (хостить Linux-драйверы в
   userspace-компоненте через шим). GPU-3D = порт Linux DRM+Mesa (гора). TCP (настоящая сеть).
