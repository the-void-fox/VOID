...
- Аллокатор фреймов **не уважает карту памяти** (bump по ОДНОМУ диапазону поверх низкой RAM; всё,
  что выше PCI-дыры 3–4 ГиБ, не используется вовсе). После Вехи 87 это ЕДИНСТВЕННОЕ, что держит
  x86 на 2047 МиБ при `-m 8G`. **План: Веха 88** — карта как список регионов.
