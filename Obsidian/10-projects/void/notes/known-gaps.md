...
Честная инвентаризация «недоделанного» на конец Вехи 49 (папки, редактор строки, освобождение фреймов, AHCI-диск, установка на диск, e1000-сеть взяты).
...
4. Крупное: драйверы реального железа — покрывают ~80% PC своими силами:
   ...
   - **USB-стек** (xHCI+core+HID) — клава/мышь/флешки; legacy-обход уже работает. ← следующее
   - Драйвер NIC ноутбука (Atheros/Realtek), NVMe, UEFI-GOP (framebuffer). Длинный хвост
     (wifi/GPU) — Genode-стиль (хостить Linux-драйверы в userspace через шим). GPU-3D = DRM+Mesa (гора).
