...
> **`reader.rs` — сменный слой, и замена СОСТОЯЛАСЬ (Веха 102, [[0013-vvsh-surface-syntax]]).**
> S-выражения заменены инфиксным синтаксисом семьи Rust/Python; поменялись ровно два места —
> `reader.rs` (вход) и печать (выход, `Display` зовёт принтер ридера). `Value`, `eval.rs`,
> нормализатор и ядро не тронуты: они работают с ДЕРЕВОМ. Граница себя оправдала — держать её
> чистой и дальше: **новый код не должен разбирать текст нигде, кроме `reader.rs`**.
...
f(a, b)                 вызов                  → (f a b)
[a, b]                  список                 → (list a b)
имя = выражение         определение            → (define имя выражение)
|x, y| тело             лямбда                 → (lambda (x y) тело)
if c { a } else { b }   ветвление              → (if c a b)
a + b · a == b          инфиксные операторы    → (+ a b) · (= a b)
a |> f(b)               конвейер (thread-last) → (| a (f b))
true / false            булевы
# до конца строки       комментарий
```

Разбор — Пратт, приоритеты снизу вверх: `|>` → `==` → `+ -` → `* /` → вызов. Три следствия
выбора: равенство `==` (одиночное `=` занято присваиванием), конвейер `|>` (одиночное `|` занято
лямбдой), булевы словами.

**Дефис — часть имени**, а не оператор: `font-size`, `split-v`, `net-srv` — так пишутся ключи
конфига, а вычитание в `.vv` почти не встречается. Цена: минус как оператор обязан отделяться
пробелами (`a - b`, не `a-b`).

**`let` и `cond` берут списки, а не голые пары**: `let([[x, 2]], x + y)`, `cond([тест, знач],
[else, знач])` — голых пар новый ридер породить не может. Особенно показателен `cond`: пока
спец-форма этого не знала, головной символ `list` сам сходил за тест (символ встроенной функции
истинен) и срабатывала первая же ветвь.

**В шелле выражение открывает ведущий `\`** — `\2 * 3 + 4`, `\map(|x| x * x, [1, 2, 3])`.
Скобка признаком быть перестала: в новом синтаксисе она означает ВЫЗОВ, и `ls(…)` неотличимо от
команды `ls`. Всё, что не начинается с `\`, — команда (голые слова), как и было.
