...
> **`reader.rs` — сменный слой.** Принято ([[0013-vvsh-surface-syntax]]) заменить S-выражения
> инфиксным синтаксисом. Про круглые скобки знают ровно два места: `reader.rs` (165 строк, вход) и
> `Display` в `value.rs` (~40, выход). Всё, что ниже — `Value`, `eval.rs`, нормализатор — работает
> с деревом и синтаксиса не знает. Держать эту границу чистой = держать замену дешёвой: **новый
> код не должен разбирать текст нигде, кроме `reader.rs`**.
