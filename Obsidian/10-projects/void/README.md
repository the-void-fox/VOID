...
## Статус
**Фазы 0–3 закрыты (Вехи 1–22); Фаза 4 идёт (Вехи 23–31 закрыты).** Тезис ADR 0002 полон,
...
последний «до железа» пункт из анализа [[commit-policy]] закрыт.
...
→ std-порт (готово) → uutils (готово) → политика коммитов (готово). Дальше по
принятому порядку: virtio-net → потоки (sort/ripgrep) → nixpkgs-cross →
