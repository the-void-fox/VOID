...
## Статус
**Фазы 0–3 закрыты (Вехи 1–22); Фаза 4 идёт (Вехи 23–34 закрыты).** Тезис ADR 0002 полон,
...
на обеих архитектурах (микроядерность: протоколы не в ядре).
...
(готово). Дальше по принятому порядку: потоки (sort/ripgrep) → nixpkgs-cross →
бэкенды B/C → checkpoint → декларативный init → реальное железо.
