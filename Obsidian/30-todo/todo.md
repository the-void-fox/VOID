...
- [x] **Веха 22 — куча процесса + честные page fault** — **закрыта**, Фаза 3 завершена. См. [[process-heap]].
    - [x] 22.1 — `SYS_MAP(len)` (syscall 17): ленивый резерв [0x6000_0000, heap_brk), 0 фреймов сразу; ELF ограничен снизу кучи.
    - [x] 22.2 — page fault из U: в куче — обнулённый фрейм + повтор инструкции; вне — гибель ПРОЦЕССА (родителю MAX), ядро живёт. Доотображение под шлюзы OBJ_GET/PUT (S-фолт фатален).
    - [x] 22.3 — персоналия на куче: 16 файлов × 4 КиБ (было 4 × 256), метаданные тоже в куче (обход memset-мины `.user`), скретч для stat (закрыт скрытый оверфлоу 512-байтного буфера ответа).
    - [x] 22.4 — демо heap (ленивые страницы, нули) + crash (store-фолт → убит, ядро живо). Грабли `.user`: memset на init массивов, jump-таблицы на -O1 (opt-level не поднимать), write_volatile не инлайнится.

## Скоро
- [ ] Веха 23 — userspace целиком в ELF из store: переезд vsh/серверов из секции `.user`, её вывод из эксплуатации (снимает минное поле memset/jump-таблиц и ограничения IPC-буферов в ленивой куче).
