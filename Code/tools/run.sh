...
# ЖИВОЙ ISO этим путём не готовится — и не должен. Компакт-диск не записывают: ядро в него
# кладёт `mkboot.sh`, вместе с образом-модулем внутри. Раньше `--img void.iso` попадал сюда как
# обычный образ диска: mcopy в него не проходил, срабатывала ветка «пересобрать с нуля», и на
# месте девяностомегабайтного ISO оказывался семисотмегабайтный образ диска. То есть команда
# ПРОВЕРИТЬ образ его молча уничтожала — ровно перед тем, как отдать людям.
case "$img" in
    *.iso)
        if [ "$build" = 1 ] || [ "$fresh" = 1 ]; then
            echo "  !! $img — живой ISO, а он не собирается и не правится этим скриптом."
            echo "     Пересобрать:  nix-shell shell.nix --run 'Code/boot/mkboot.sh'"
            echo "     Проверить:    Code/tools/run.sh --no-build --img $img …"
            exit 1
        fi
        say "живой ISO — беру как есть (ядро внутри уже своё, записывать нечего)"
        ;;
    *)
[ "$fresh" = 1 ] && rm -f "$img"
if [ ! -f "$img" ]; then
...
if [ -n "$script" ]; then
    ...
    if [ "$tcg" = 0 ] && [ -r /dev/kvm ] && [ -w /dev/kvm ]; then
        ...
        # `-cpu host`, как и в обычном запуске. Умолчание QEMU (`qemu64`) — процессор, которого
        # не существует: в нём нет ни SMEP, ни TSC-deadline, ни половины того, что есть на любом
        # живом железе. Проверка на нём молча идёт запасным путём и говорит «работает» про код,
        # который на машине владельца не исполнится вовсе (Веха 145.4 — ровно этот случай).
        export VOID_QEMU_CPU="${VOID_QEMU_CPU:-host}"
    else
