...
## Случайность (Веха 86 → три источника перед Вехой 95)
На загрузке ядро печатает, что реально нашлось:
```
[rng]  источники: virtio-rng есть · ГСЧ процессора нет · джиттер есть
[rng]  семя прошлой загрузки подмешано в пул
```
- **virtio-rng** — энтропия от гипервизора (раннеры его поднимают). Нужен по-настоящему:
  **QEMU по умолчанию даёт модель CPU без `RDRAND`** (проверяется `-cpu max`).
- **ГСЧ процессора** — `RDRAND`, есть с Ivy Bridge (2012).
- **джиттер** — разброс времени исполнения на самом процессоре; **единственный источник,
  работающий без гипервизора и без `RDRAND`**, то есть на машинах старше 2012 года.
- **семя** в сторе (`system/random-seed`) переносит энтропию через перезагрузку.

Источники **смешиваются**, а не выбираются: слабость любого не портит выход. Если сильного
источника не нашлось ни одного, ядро говорит это прямым текстом — ключи на такой машине
генерировать нельзя.
