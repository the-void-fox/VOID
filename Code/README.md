...
## Случайность (Веха 86 → virtio-rng перед Вехой 95)
Раннеры поднимают `virtio-rng` — источник энтропии от гипервизора. Он нужен по-настоящему:
**QEMU по умолчанию даёт модель CPU без `RDRAND`** (проверяется `-cpu max`), так что без этого
устройства криптографического источника нет ни на riscv, ни на x86 — только пул джиттера
прерываний. Что нашлось на самом деле, ядро печатает на загрузке строкой `[rng]`; на этой
строке и стоит сверяться перед разговором про TLS.
