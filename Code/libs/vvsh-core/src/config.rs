...
//! терминалу (`bin/term` читает поколение из store сам). Разделять их на два объекта не за что:
//! конфигурация системы — ОДНА вещь, у неё одна история поколений и один откат; кто какие строки
//! берёт — дело читателя, а не хранилища.
