...
static PROGRAMS: &[(&str, &[u8])] = &[
    ...
    // В общем списке: сборка это чужой процесс и файлы, экрана ей не нужно.
    ("nixb", include_bytes!(env!("PROG_NIXB"))),
    // Веха 188 (ADR 0019, шаг 5) — ВЫЧИСЛИТЕЛЬ ЯЗЫКА NIX: считает выражение и печатает значение.
    ("nixe", include_bytes!(env!("PROG_NIXE"))),
];
...
static PROGRAMS_ARCH: &[(&str, &[u8])] = &[
    ...
    // Веха 172 — окно «добро пожаловать». Запускает его не ядро и не композитор по особому
...
/// Веха 170, этап 4 — **замок вывода ядра**: консоль и кольцо журнала.
///
/// Печать до этой вехи защищалась одним лишь запретом прерываний — этого хватало, потому что
/// исполнение было одно. Сейчас почти всякий `println!` ядра и так идёт под большим замком, и
/// можно было бы на это положиться. Но «положиться» здесь означает «помнить»: каждый новый
/// `println!` в пути, где большого замка нет, молча ломал бы вывод по буквам, а найти такое в
/// перемешанном тексте почти невозможно (одна такая строка уже стоила захода в этапе 2).
/// Замок стоит одного `compare_exchange` на печать — и снимает необходимость помнить.
pub(crate) static OUT: sync::SpinLock<()> = sync::SpinLock::new(());

/// Веха 170 — отпустить [`OUT`] за упавшего. Только для аварийного пути: ядро могло упасть
/// посреди печати, и тогда сообщение об аварии встало бы на его же замке — то есть не появилось
/// бы нигде.
pub(crate) fn break_output_lock() {
    // SAFETY: зовётся из аварийного пути, когда соседи уже остановлены ([`cpu::stop_others`]) и
    // целостность вывода больше никому не понадобится.
    unsafe { OUT.force_unlock() };
}
...
pub extern "C" fn kmain(hartid: usize, dtb: usize) -> ! {
    ...
    // Веха 7.1/47: подключить диск (нужен для персистентности). На обычной загрузке сперва AHCI —
    // так стоит диск на реальном x86-железе (SATA); в QEMU virt/q35 его нет → откат на virtio-blk.
    //
    // Веха 174.1 — но **на ЖИВОМ НОСИТЕЛЕ памяти отдаётся предпочтение перед любым диском**, и
    // это не мелочь порядка. Признак живого носителя — загрузочный модуль GRUB с образом
    // ([`arch::boot_module`]); значит человек загрузился с флешки, а не с диска. Пока правило
    // было «настоящий диск всегда лучше памяти», такая загрузка на машине с УЖЕ УСТАНОВЛЕННЫМ
    // VOID подхватывала store установленной системы: живой сеанс начинал писать в неё поколения,
    // а установщик отказывался ставить на этот диск — «с него работает система». Владелец это и
    // увидел: поставить новую версию поверх старой было нельзя вовсе.
    //
    ...
    if cores > 1 && up == cores {
        println!("  [smp]  ядер {cores} — планировщик работает на всех");
    } else if cores > 1 {
        // Подняли не все: либо упёрлись в свой потолок, либо так попросила строка загрузки
        // (`cores=N`), либо ядро не ответило на SIPI. Причину назвал тот, кто её знает, — выше.
        println!("  [smp]  ядер {cores}, поднято {up} — планировщик работает на них");
    } else {
...
fn panic(info: &PanicInfo) -> ! {
    // Веха 170 — остановить остальные ядра ПЕРВЫМ делом. Пока они работают, они исполняют
    // процессы поверх ядра, которое уже объявило себя сломанным, и пишут в ту же консоль:
    // сообщение о панике вышло бы вперемешку с обычным выводом. И пробить замок вывода — упасть
    // мы могли ровно посреди печати, а тогда сказать о панике было бы негде.
    cpu::stop_others();
    break_output_lock();
    ...
    println!();
    println!("  [PANIC] {}", info);
    // Именно остановка, а не сон до прерывания: разбуженное ядро в аварии продолжало бы
    // крутить обработчики поверх недостоверного состояния.
    cpu::halt_here()
}
