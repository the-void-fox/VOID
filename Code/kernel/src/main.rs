...
pub extern "C" fn kmain(hartid: usize, dtb: usize) -> ! {
    ...
    //
    // Печатаем ОБА числа, потому что они могут разойтись: сколько ядер у машины — свойство
    // машины, сколько поднято — наше (потолок `MAX_CPUS`, отказ ядра ответить на SIPI).
    // Поднятые ядра ждут открытия сессии процессов и дальше планируют наравне с этим.
    // SAFETY: зовётся один раз, из загрузочного ядра, после подъёма LAPIC.
    let (cores, up) = unsafe { arch::start_aps() };
    if cores > 1 {
        println!("  [smp]  ядер {cores}, поднято {up} — планировщик работает на всех");
    } else {
