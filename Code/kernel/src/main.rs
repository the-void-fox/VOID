...
pub extern "C" fn kmain(hartid: usize, dtb: usize) -> ! {
    ...
    // Веха 170 — ПРИКЛАДНЫЕ ЯДРА. Строго после `timer::init`: он поднимает локальный APIC
    // загрузочного ядра, а межпроцессорные прерывания живут в том же блоке регистров.
    //
    // Поднятые ядра пока паркуются (см. `arch::x86_64::smp`): планировщик, замок ядра и
    // рассылка сброса TLB — следующие этапы. Печатаем ОБА числа, потому что разница между
    // «машина многоядерная» и «система многоядерная» — это и есть то, что сейчас правда.
    // SAFETY: зовётся один раз, из загрузочного ядра, после подъёма LAPIC.
    let (cores, up) = unsafe { arch::start_aps() };
    if cores > 1 {
        println!("  [smp]  ядер {cores}, поднято {up} (работает на одном — планировщик пока свой)");
    } else {
        println!("  [smp]  ядро одно");
    }
