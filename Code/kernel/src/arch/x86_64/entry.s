...
boot_magic:                         # magic загрузки (eax при входе): 0x2BADB002 = multiboot
    .space 8
.align 4096
# .global — трамплин прикладного ядра (ap.s, Веха 170) входит в длинный режим ЭТИМИ же
# таблицами: строить ему второй комплект значило бы завести вторую истину о раскладке памяти.
.global boot_pml4
boot_pml4:
    .space 4096
boot_pdpt:
...
.align 16
# .global — не украшение: границы стека читает аварийный дамп (trap.rs, Веха 126.4), а он
# компилируется в СВОЮ единицу трансляции. Пока метки были локальными, ссылка на них
# разрешалась лишь по счастью — в release, где единица одна; debug-сборка не линковалась вовсе.
.global _boot_stack
