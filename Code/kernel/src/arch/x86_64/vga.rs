...
//! экрана реальной машины) — оба дёшевы. Курсор — статик; гонок за него нет, потому что весь
//! вывод ядра идёт под замком [`crate::OUT`] (Веха 170; до неё хватало запрета прерываний —
//! исполнение было одно).
//!
...
fn map_cp866(c: char) -> u8 {
    match c {
        ...
        '→' => 0x1A, '←' => 0x1B, '↑' => 0x18, '↓' => 0x19,
        '…' => b'.',
        // Знаки, которых в CP866 нет, но которые система печатает о себе сама: размеры экрана
        // («1280×800») и сетки. Без них в собственном баннере стоял вопросительный знак.
        '×' => b'x',
        '≈' => b'~',
        '№' => b'#',
        _ => b'?',
