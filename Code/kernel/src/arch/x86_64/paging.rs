...
/// Веха 129 — **программный** бит: страница ОБЩАЯ, её фреймы принадлежат области разделяемой
/// памяти, а не этому процессу. Биты 9–11 в записи процессор не трогает и оставляет системе —
/// ровно для таких пометок.
///
/// Без него смерть процесса освобождала бы чужие страницы: `free_private` отдаёт в список
/// свободных всякий лист, который лежит в RAM, и общий буфер окна ушёл бы туда вместе с
/// приватными — а второй держатель продолжал бы в него писать. Такую порчу ищут неделями.
pub const PTE_SHARED: u64 = 1 << 9;
/// Бит PAT в записи ЛИСТА 4 КиБ (в PD/PDPT тот же бит означает «это лист» — см. `PTE_PS`).
/// Вместе с PCD/PWT он выбирает строку таблицы PAT; мы пользуемся ровно одной — WC (Веха 117).
pub const PTE_PAT: u64 = 1 << 7;

/// Веха 117 — **write-combining для фреймбуфера**.
///
/// Замер на X54C: 24 МБ/с, полный кадр 1024×768 — 131 мс, то есть 166 нс на каждую 4-байтную
/// запись. Это подпись НЕКЭШИРУЕМОГО (UC) отображения: каждая запись едет отдельной транзакцией
/// по PCIe. Виноваты не таблицы страниц — там обычный WB, — а MTRR прошивки, которая помечает
/// область PCI как UC; итоговый тип берётся строже.
///
/// PAT — единственный способ это перебить: по таблице Intel SDM «MTRR = UC + PAT = WC» даёт
/// **WC**. Тогда процессор копит записи в буфере и отдаёт их пачками по 64 байта.
///
/// Программируем ОДНУ строку — PA4 (индекс выбирается битом PAT при PCD=0, PWT=0) — в WC,
/// остальные оставляем как есть. Так ни одно существующее отображение не меняет смысла: всё,
/// что не просит PTE_PAT, живёт по-прежнему.
const IA32_PAT: u32 = 0x277;
const PAT_WC: u64 = 0x01;

/// Записать в PAT строку WC. Зовётся на загрузке, ДО того как кто-либо просит WC-страницу.
///
/// Честная оговорка: SDM предписывает для смены PAT целый обряд (выключить кэш, сбросить,
/// сменить, инвалидировать TLB). Мы делаем это ОДИН раз на единственном процессоре, до того как
/// появилось хоть одно WC-отображение, — то есть менять смысл уже существующих страниц не
/// приходится. Появится SMP — обряд придётся исполнить целиком, и на каждом ядре.
pub fn pat_init() {
    unsafe {
        let (lo, hi): (u32, u32);
        ...
        pat &= !(0xffu64 << 32); // PA4 — байт №4
        pat |= PAT_WC << 32;
        let (lo, hi) = (pat as u32, (pat >> 32) as u32);
        core::arch::asm!("wrmsr", in("ecx") IA32_PAT, in("eax") lo, in("edx") hi,
                         options(nomem, nostack));
    }
}
...
pub fn init() -> usize {
    ...
    if enable_smep() {
        crate::println!("  [vm]   SMEP включён — ядру запрещено исполнять страницы процессов");
    }
    root
}

/// Веха 128 — **SMEP**: процессор запрещает ЯДРУ выбирать инструкции из страниц с битом U.
///
/// Зачем это нам, если ядро и так не исполняет чужие страницы. Затем, что «не исполняет» —
/// это утверждение о коде, а SMEP делает его свойством железа. Веха 126.4 показала, чем
/// оборачивается разница: ядро ушло по нулевому адресу и мы его поймали лишь потому, что нулевая
/// страница не отображена. Уйди оно по адресу, где лежит код процесса, — исполнение молча
/// продолжилось бы в чужом коде с правами ядра, и это уже не падение, а захват.
///
/// Включать безопасно: и трамплин (`entry.s`, записи `0x83` = P|RW|PS), и окно образа ядра
/// отображены БЕЗ бита U — исполнять нам нечего.
///
/// `false` — процессор SMEP не умеет (до Ivy Bridge; X54C — Sandy Bridge, там его нет).
fn enable_smep() -> bool {
    // CPUID лист 7, подлист 0: бит 7 в EBX — SMEP. `rbx` в `asm!` зарезервирован LLVM, поэтому
    // сохраняем его руками — обычный приём для cpuid.
    let ebx: u32;
    unsafe {
        core::arch::asm!(
            "push rbx",
...
/// Включить трансляцию по нашим таблицам: загрузить CR3, если он ещё не тот.
///
/// ## Почему «если ещё не тот» — это не микрооптимизация (Веха 170.6)
///
/// Запись CR3 сбрасывает ВЕСЬ буфер трансляций. А зовётся это на каждом входе в процесс, то есть
/// на каждом системном вызове и каждом тике вытеснения — и почти всегда с тем же самым корнем,
/// что уже стоит. То есть система на ровном месте выбрасывала весь TLB тысячи раз в секунду,
/// после чего композитор заново обходил таблицы для своего фреймбуфера и буферов окон. В QEMU
/// этого почти не видно (там трансляции кэширует хозяин), а на настоящей машине это и есть
/// «любое изменение экрана даётся с трудом» — владелец увидел ровно это.
///
/// Раньше на этот сброс молча опирались: отобразил страницу — TLB всё равно сбросится на входе в
/// процесс. Больше не опираются, и это условие Вехи 170: КАЖДОЕ изменение отображений живого
/// пространства сбрасывает буфер само — `handle_user_fault` и `ensure_heap_range` своим
/// `flush_tlb`, всё остальное (снятие, перезапись, урезание прав) — `proc::flush_space`, которая
/// вдобавок рассылает сброс соседям. Отображения в НЕ живое пространство (загрузка ELF, разморозка
/// снимка) сбрасывать нечего: их корень ни у кого в CR3, а первый вход в такой процесс корень
/// сменит — и сбросит.
///
/// # Safety
...
pub unsafe fn enable(root_pa: usize) {
    let cur: usize;
    core::arch::asm!("mov {0}, cr3", out(reg) cur, options(nomem, nostack, preserves_flags));
    // Младшие 12 бит CR3 — флаги кэширования; корень сравниваем по адресу.
    if cur & !0xfff == root_pa & !0xfff {
        return;
    }
...
unsafe fn free_private(tbl: usize, ktbl: usize, level: usize) {
    ...
    for i in 0..512 {
        ...
        // Веха 126.4 — «общая с ядром» значит УКАЗЫВАЕТ НА ТУ ЖЕ ТАБЛИЦУ, а не «побитово равна».
        //
        // Прежнее сравнение целых записей (`pte == *k.add(i)`) опиралось на то, что копия
        // ядерной записи в таблице процесса вечно совпадает с оригиналом. Это неверно в
        // принципе: биты Accessed/Dirty выставляет САМ процессор при обращении, и записи
        // расходятся ими сами собой. Разойдясь, запись перестала бы считаться общей — и уборка
        // спустилась бы в таблицы ЯДРА и освободила их. Замер (Веха 126.4) показал, что сегодня
        // они не расходятся, то есть это не причина известных падений; но правило не должно
        // зависеть от везения, а сравнение по адресу и есть настоящий смысл «та же таблица».
        let kpte = if ktbl != 0 { *k.add(i) } else { 0 };
        if kpte & PTE_P != 0 && (kpte & ADDR_MASK) == (pte & ADDR_MASK) {
            continue; // та же таблица/страница, что у ядра — не наша, не трогаем
        }
        ...
        // Веха 129 — общая страница: фреймы принадлежат области разделяемой памяти, и
        // освобождает их она (по счётчику держателей), а не умирающий процесс.
        if level == 0 && pte & PTE_SHARED != 0 {
            continue;
        }
