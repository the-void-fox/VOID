...
pub use paging::{clone_kernel_root, copy_user_space, free_address_space, translate};
...
/// Веха 136 — ЧАСТОТА `rdtsc`, измеренная по PIT. Раньше её не измеряли вовсе: в ядре стояло
/// «TSC ≈ 1 ГГц в QEMU TCG», и это было единственной правдой о времени во всей системе.
///
/// Под KVM и на живом железе TSC идёт с частотой процессора — здесь это оказалось 3 ГГц, то есть
/// **всё время VOID шло втрое быстрее настоящего**: `date` убегал, сроки истекали втрое раньше
/// заявленных, а замеры врали втрое в другую сторону.
///
/// Способ — классический (тот же, что в Linux `pit_calibrate_tsc`): канал 2 PIT считает от
/// известной частоты 1.193182 МГц, мы держим окно и считаем, сколько за него натикал TSC.
/// CPUID не годится опорой: leaf 0x15/0x16 есть не у всех, а под гипервизором врёт чаще, чем
/// молчит. PIT есть и в q35, и на любом ноутбуке этого класса.
///
/// `None` — калибровка не удалась (PIT не отвечает: выход из счёта не наступил за разумное
/// число тактов, либо получилась нелепая частота). Тогда зовущий берёт прежнюю константу и
/// ЗНАЕТ, что время приблизительное, — а не думает, что оно точное.
pub fn timer_hz() -> Option<u64> {
    const PIT_HZ: u64 = 1_193_182;
    /// Окно калибровки ≈ 20 мс. Длиннее — точнее, но это чистая задержка загрузки.
    const LATCH: u16 = (PIT_HZ * 20 / 1000) as u16;
    /// Предохранитель: если бит окончания счёта не встал за столько тактов, PIT'а нет и ждать
    /// его нельзя — иначе загрузка встанет насмерть на машине без канала 2.
    const GIVE_UP_CYCLES: u64 = 40_000_000_000;

    // Ворота канала 2 поднять, динамик отцепить (бит 1) — иначе калибровка ПИЩИТ.
    outb(0x61, (inb(0x61) & !0x02) | 0x01);
    // Канал 2, доступ lo/hi, режим 0 (сигнал по достижении нуля), двоичный счёт.
    outb(0x43, 0xb0);
    outb(0x42, (LATCH & 0xff) as u8);
    outb(0x42, (LATCH >> 8) as u8);

    let t0 = now_ticks();
    while inb(0x61) & 0x20 == 0 {
        if now_ticks().wrapping_sub(t0) > GIVE_UP_CYCLES {
            return None;
        }
    }
    let elapsed = now_ticks().wrapping_sub(t0);
    let hz = elapsed.saturating_mul(PIT_HZ) / LATCH as u64;
    // Здравый смысл: TSC медленнее 10 МГц или быстрее 100 ГГц — это не измерение, а мусор.
    (10_000_000..100_000_000_000).contains(&hz).then_some(hz)
}

/// Веха 35 — монотонный счётчик тиков для futex-дедлайнов (`rdtsc`; частоту измеряет
/// [`timer_hz`]). Тот же счётчик, что читает U-mode для замеров и `Instant`.
