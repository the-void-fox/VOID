...
/// Веха 170, этап 2 — обычное межпроцессорное прерывание: разбудить ядро `dest` вектором
/// `vector`.
///
/// Режим доставки Fixed (000), назначение по физическому идентификатору APIC. Ждём ухода
/// предыдущего ([`icr_wait`]) — регистр команд один на ядро, и запись поверх неотправленной
/// команды теряет её молча.
///
/// # Safety
/// Зовётся с поднятым LAPIC; `vector` обязан иметь шлюз в IDT — прерывание на вектор без шлюза
/// не игнорируется, а убивает машину.
pub unsafe fn send_ipi(dest: u8, vector: u8) {
    icr_send(dest, vector as u32);
}
