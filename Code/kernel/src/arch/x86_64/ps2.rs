...
/// Веха 143 — какая раскладка сейчас: 0 — US, 1 — RU.
pub fn keymap() -> usize {
    unsafe { RU as usize }
}

/// Веха 143 — переключить раскладку (`n` — номер). Зовётся системным вызовом от того, кому
/// принадлежит экран: аккорд переключения задаётся конфигом, а таблицы живут здесь.
pub fn keymap_set(n: usize) {
    unsafe { RU = n != 0 }
}

/// Сколько раскладок знает драйвер.
pub const KEYMAPS: usize = 2;
