...
/// Направить GSI на вектор: fixed, physical, edge, active-high, без маски, ЗАГРУЗОЧНОЕ ядро.
///
/// Веха 170 — адресат назывался числом 0, и это работало, потому что у загрузочного ядра QEMU
/// идентификатор APIC ноль. Совпадение, а не правило: прошивка вольна дать ему любой номер, и
/// тогда прерывания устройств уезжали бы на прикладное ядро. Работать система при этом
/// продолжила бы (в ядре всё под большим замком), но спящий в `SYS_READ` ждал бы своего байта,
/// пока кто-нибудь не освободится: разбудить его умеет только загрузочное ядро.
pub fn route(gsi: u32, vector: u8) {
    write(0x10 + 2 * gsi, vector as u32);
    write(0x11 + 2 * gsi, boot_dest());
}

/// Адресат прерываний устройств — идентификатор APIC ЗАГРУЗОЧНОГО ядра, в поле destination
/// (биты 31..24 старшего слова RTE). Маршрутизацию ставит оно само на загрузке, поэтому «своё»
/// и «загрузочное» здесь одно и то же.
fn boot_dest() -> u32 {
    (super::lapic::id() as u32) << 24
}
