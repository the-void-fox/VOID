...
///
/// # Safety
/// Зовётся из консоли под замком вывода [`crate::OUT`] — гонок за тень нет. Веха 170: до неё
/// довольно было запрета прерываний, потому что исполнение было одно; теперь запрет прерываний
/// не говорит ничего о соседнем ядре.
