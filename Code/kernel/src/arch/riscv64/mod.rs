...
/// Веха 120 — размера у консоли нет: она здесь UART, а сколько знакомест у ТЕРМИНАЛА на другом
/// конце провода, ядро не знает и знать не может. Нули значат «не спрашивай меня» — программа
/// возьмёт своё умолчание, а не поверит выдумке.
...
/// Веха 143 — раскладок на riscv нет: клавиатура приходит по UART готовыми байтами, и менять
/// в них нечего. Честная заглушка, а не «одна раскладка US».
pub fn keymap() -> usize {
    0
}
pub fn keymap_set(_n: usize) {}
...
/// Веха 145.4 — разбудить В ЭТОТ момент (`now_ticks`, то есть счётчик `time`).
///
/// Здесь это одна строка: SBI TIME и так берёт АБСОЛЮТНЫЙ момент, а сроки ядра живут в тех же
/// тиках. Квант вытеснения был единственной причиной, по которой сюда всегда передавали
/// «сейчас + 20 мс», — и он же заставлял спящего по сроку ждать до конца кванта.
...
/// Веха 170 — попросить ядро `i` сбросить буфер трансляций. Некого: харт один, и свой буфер он
/// сбрасывает сам, `sfence.vma`.
pub fn flush_cpu(_i: usize) {}
