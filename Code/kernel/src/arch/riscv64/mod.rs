...
///
/// Пока **0**: отображение остаётся тождественным, и весь код ведёт себя ровно как раньше. Смысл
/// этапа — развести понятия «физический адрес» и «указатель, по которому ядро к нему обращается»:
/// каждое такое место теперь проходит через [`phys_to_virt`]. Когда ядро переедет в верхнюю
/// половину, поменяется ТОЛЬКО эта константа и путь загрузки, а не сотня мест по всему ядру.
