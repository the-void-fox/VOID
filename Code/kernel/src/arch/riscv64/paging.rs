...
/// Включить трансляцию: `satp = Sv39 | PPN(корень)`, затем сбросить TLB — но только если корень
/// СМЕНИЛСЯ.
///
/// Веха 170.6 — зеркало `x86_64::paging::enable`, и по той же причине: зовётся это на каждом
/// входе в процесс, а `sfence.vma` выбрасывает весь буфер трансляций. Условие, на котором такой
/// пропуск законен, описано там же — каждое изменение отображений живого пространства сбрасывает
/// буфер само.
///
/// # Safety
/// Вызывать, когда таблицы уже построены и идентично отображают текущие PC/SP/стек, иначе
/// следующая же инструкция уйдёт в page fault.
pub unsafe fn enable(root_pa: usize) {
    let satp = (8usize << 60) | (root_pa >> 12); // MODE=8 (Sv39), ASID=0, PPN корня
    let cur: usize;
    asm!("csrr {0}, satp", out(reg) cur, options(nomem, nostack));
    if cur == satp {
        return;
    }
    asm!(
