...
fn wait_stdin(saved_sie: usize) -> bool {
    ...
    // которые некому прочитать в ГРАФИЧЕСКОМ сеансе.
    //
    ...
    //
    // Бросаем, только если в `SYS_READ` не спит НИКТО: тогда байт и правда некому отдать (он
    // не разбудит даже композитор — того будит событие клавиши, а не байт). Если кто-то ждёт
    // ввода, кольцо не трогаем: там набранное впрок, и оно дойдёт до читателя.
    if waiting.is_empty() && arch::video_owner().is_some() {
