...
/// процесс.
///
/// У ядерных НИТЕЙ сторож есть с Вехи 89 ([[scheduling]]), а у стека, на котором исполняются ВСЕ
/// syscall'ы, не было ничего — при том что прямо под ним в `.bss` лежит [`RETURN_CTX`]. Тихое
/// переполнение затёрло бы его, и выход из сессии процессов ушёл бы в никуда — авария далеко от
/// причины, ровно как в Вехе 126.4.
///
/// Значение то же, что у нитей: одна магия на все стеки ядра — искать её в дампе проще, чем
...
struct Proc {
    ...
    wake_on_key: bool,
    /// Веха 141.1 — ЧИТАЕТ ЛИ процесс байтовое кольцо консоли (`SYS_READ`). Узнаётся по факту
    /// первого чтения: спросить об этом заранее не у кого, а разница между читателем и
    /// нечитателем — единственное, что отличает текстовый сеанс от оконного (см. `wait_stdin`).
    reads_console: bool,
...
fn idle_ticks_avg() -> u64 {
    let sum: u64 = IDLE_TICKS.iter().map(|c| c.load(Ordering::Relaxed)).sum();
...
pub fn run_ap() -> ! {
    ...
    loop {
        ...
        release_space(me);
        // И замолчать таймеру: будить спящее ядро двадцать раз в секунду, чтобы оно каждый раз
        // убеждалось, что работы нет, — это ровно та трата, ради устранения которой ядро и
        // засыпает. Разбудит нас сосед побудкой ([`cpu::wake_one`]).
        arch::irq_mask_write(0);
        let idle_from = arch::now_ticks();
        cpu::set_idle(true);
        // Сказать загрузочному ядру, что мы освободились. Только оно умеет решать, что делать
        // дальше: заметить ближайший срок, подобрать ввод с консоли, закончить сессию. Пока мы
        // работали, оно могло уснуть с картиной мира, в которой всё занято, — а она изменилась.
        cpu::wake_if_idle(0);
        cpu::unlock();
        arch::wait_for_interrupt();
        cpu::set_idle(false);
        IDLE_TICKS[me].fetch_add(arch::now_ticks().saturating_sub(idle_from), Ordering::Relaxed);
    }
}
...
fn wait_stdin(saved_sie: usize) -> bool {
    let (waiting, irq_waiting, deadline, wants_input, console_reader): (
        Vec<usize>,
        bool,
        Option<u64>,
        bool,
        ...
        //
        // Веха 141 — срок берётся ТОЛЬКО у тех, кого он и разбудит ([`sleeps_by_deadline`]).
        // Раньше сканировались все процессы подряд, и хватало одного мёртвого со сроком на
        // руках: `wake_futex_timeouts` чужие состояния пропускает и поле не чистит, поэтому в
        // таблице оставался ВЕЧНО ИСТЁКШИЙ срок. Условие сна ниже проверяет `now >= d` и с таким
        // сроком не засыпало НИКОГДА — система жгла целое ядро на пустом экране. Ловится это
        // только счётчиком процессора: работать при этом всё продолжает правильно.
        let d = t.procs.iter().filter(|p| sleeps_by_deadline(p)).filter_map(|p| p.futex_deadline).min();
        ...
        // Веха 141.1 — есть ли КОМУ забрать байты консоли. Не «владеет ли кто-то экраном»
        // (первый заход этой починки), а именно «читает ли их кто-нибудь»: полноэкранный `term`
        // экраном владеет И читает консоль, поэтому по владению у него отбирали набранное —
        // ввод в текстовом режиме начал пропадать через раз.
    ...
    // Веха 141 — ЗЕРКАЛЬНЫЙ случай той же ошибки, только в другую сторону: байты консоли,
    // которые некому прочитать в ОКОННОМ сеансе.
    //
    // Клавиатура кладёт каждое нажатие в оба кольца сразу: в очередь событий (её читает
    // композитор) и в байтовое кольцо консоли (его читает `SYS_READ`). В оконном режиме
    // `SYS_READ` не зовёт никто — программы окон говорят через stdio по IPC, — и кольцо
    // консоли копило байты навсегда. А `console_has_input()` ниже — условие выхода из сна:
    // после ПЕРВОГО же нажатия система переставала спать вовсе и жгла целое ядро (замер:
    // 100% против 0.2% до нажатия). Ничего при этом не ломалось — цена видна только счётчиком
    // процессора, ровно как в Вехе 139.3.
    //
    // Бросаем ровно тогда, когда кольцо не читает НИКТО (Веха 141.1). Первым признаком было
    // «экран принадлежит процессу» — и это оказалось неверно: полноэкранный `term` тоже владеет
    // экраном, но байты консоли читает именно он. Байт мог прийти за миг до захода сюда, мы его
    // выбрасывали, а `term` будило событие клавиши — тот шёл читать и не находил ничего. Со
    // стороны это «система иногда съедает нажатия», и только в текстовом режиме.
    if !console_reader {
        while arch::console_getc().is_some() {}
    }
        ...
        && !any_runnable_for_me()
    {
        ...
        //
        // Раньше сюда приходили с таймером, взведённым на «сейчас + 20 мс» (это же квант
        // вытеснения), и спящий просыпался, когда придётся: замер Вехи 145.3 — просили 6.6 мс,
        // спали 10.6. Композитор из-за этого держал 50 кадров в секунду вместо 60, и подобрать
        // паузу в пользователе было нельзя в принципе — он не знает, когда его разбудят.
        //
        // Взводим на КАЖДОМ обороте, а не один раз до цикла: срабатывание таймера разоружает
        // его (в режиме TSC-deadline процессор обнуляет MSR), да и разбудить нас мог кто угодно
        // другой — устройство, клавиша. Взвод идемпотентен и стоит одной записи.
    ...
    cpu::set_idle(false);
    cpu::lock();
    IDLE_TICKS[cpu::id()].fetch_add(arch::now_ticks().saturating_sub(idle_from), Ordering::Relaxed);
    let mut t = TABLE.lock();
...
/// Спит ли процесс ПО СРОКУ, то есть разбудит ли его истёкший `futex_deadline`.
///
/// Один список состояний на оба конца: его спрашивает и тот, кто ищет ближайший срок для
/// простоя ([`wait_stdin`]), и тот, кто по сроку будит ([`wake_futex_timeouts`]). Разойдись
/// они — получится либо проспанный срок, либо холостой цикл вместо сна (Веха 141).
///
/// Живой процесс тоже может держать просроченное поле: между пробуждением по СООБЩЕНИЮ и
...
fn wake_futex_timeouts(t: &mut Table) {
    ...
    for i in 0..t.procs.len() {
        if !sleeps_by_deadline(&t.procs[i]) {
            continue;
        }
...
fn resume() -> ! {
    ...
    // Веха 128 — сторож стека trap'ов. Проверка здесь, а не на входе в trap: сюда приходят
    // ПОСЛЕ всей работы, проделанной этим вызовом, значит переполнение ловится тем же trap'ом,
    // который его допустил, а не следующим.
    assert!(trap_canary_ok(), "переполнение стека trap'ов ядра (см. proc::TRAP_CANARY)");
    ...
    match chosen {
        Some(n) => {
            ...
            // списка свободных. Ниже `enter_user` запишет тот же корень второй раз: это одна
            // запись регистра на переключение процесса, и она дешевле такой гонки.
