...
///
/// Веха 156.1 — **сперва занимаем СВОЙ ЖЕ слот, поднятый из прошлой загрузки** (`persisted`,
/// ровно та же цель и ровно те же права). Без этого таблица прав росла НА ОДИН СЛОТ ЗА ЗАГРУЗКУ:
/// `.cspace` возвращал слот с правом, init тут же минтил рядом второй такой же, а клэмп
/// наделения ([`clamp_persisted`]) первый УСЫНОВЛЯЛ — он ведь покрыт наделением. Через пять
/// загрузок у `posixfs` в графе прав диспетчера стояло пять одинаковых `store [rw]`, и это была
/// не картинка, а настоящие слоты в c-space.
///
/// Занять старый слот правильнее, чем завести новый: право то же самое, а дескриптор прошлой
/// загрузки при этом остаётся действительным — ровно то, что обещает персистентный c-space
/// (Веха 21.3). Совпадение требуется ТОЧНОЕ: другие права — другое полномочие, и подменять им
/// старый слот нельзя (иначе `store:rw` тихо стал бы `store:rwx` прошлой загрузки наоборот).
fn alloc_slot(dom: &mut Domain, entry: Entry) -> Cap {
    for (i, s) in dom.slots.iter_mut().enumerate() {
        let same = s.persisted
            && matches!(&s.entry, Some(e) if e.rights == entry.rights && target_eq(&e.target, &entry.target));
        if same {
            s.persisted = false; // усыновлён живым наделением этой загрузки
            s.noinherit = false;
...
/// Веха 152.2 — **вид, права и с кем связан** дескриптор: read-only интроспекция для
/// `SYS_CAP_INFO`.
///
/// Кода вида хватает, чтобы отличить «дотянулся до store-read, которое и так есть» от
/// «дотянулся до POWER, которого не давали». Побочного эффекта нет — это не «действие правом»,
/// а его ОПИСАНИЕ; узнать вид можно только про cap, который уже держишь. Коды видов держит
/// [`info_kind`], чтобы userspace и ядро называли их одинаково.
///
/// Веха 166 — плюс `aux`: `(вид, права, aux)`, где `aux` — процесс на
...
/// - Веха 156.1: покрыт, но ровно та же власть уже лежит в ЖИВОМ слоте → тоже отозвать. Это не
///   восстановление, а дубликат: восстанавливать нечего, когда то же право уже выдано заново.
///
...
pub fn clamp_persisted(dom: DomainId, endowment: &[Cap]) {
    ...
    let Some(d) = cs.domains.get_mut(dom) else { return };
    // Веха 156.1 — власть, уже лежащая в ЖИВЫХ слотах этой загрузки. Персистентный слот с ровно
    // такой же властью — не восстановление, а ДУБЛИКАТ: право то же самое, а строка в таблице
    // лишняя. Такие копил прежний порядок (по одной за загрузку), и вычищаются они здесь же —
    // иначе образ, уже накопивший их, так и остался бы с пятью одинаковыми `store`.
    let live: Vec<(Target, Rights)> = d
        .slots
        .iter()
        .filter(|s| !s.persisted)
        .filter_map(|s| s.entry.as_ref().map(|e| (e.target.clone(), e.rights)))
        .collect();
    for s in d.slots.iter_mut() {
        if !s.persisted {
            continue;
        ...
        match &s.entry {
            ...
            Some(e) => {
                ...
                let dup = live
                    .iter()
                    .any(|(t, r)| target_eq(t, &e.target) && *r == e.rights);
                if covered && !dup {
