...
//!
//! **Почему здесь нет ABA** (Веха 170 — переписано под многоядерность). Treiber-стек без
//! счётчика поколений корректен ровно тогда, когда между чтением головы и её заменой никто не
//! успевает снять и вернуть тот же фрейм. У нас так и есть, но причина сменилась: раньше это
//! было «ядро одно», теперь — **весь аллокатор работает под большим замком ядра**
//! ([`crate::cpu`]). Звать [`alloc`]/[`free`] можно только изнутри ядра, а туда входят по
//! одному; вложенных вызовов из прерываний не бывает по-прежнему (в ядре прерывания запрещены).
//!
//! Если однажды большой замок начнут дробить, это место — первое, куда надо посмотреть: CAS
//! здесь не защищает, он лишь выглядит защищающим.
...
/// Веха 133 — выделить `pages` подряд идущих обнулённых фреймов. Возвращает физический адрес
/// НАЧАЛА или `None`.
///
/// Зачем отдельно от [`alloc`]: кольца дескрипторов сетевой карты обязаны лежать в ФИЗИЧЕСКИ
/// НЕПРЕРЫВНОЙ памяти — карта ходит по ним сама, о таблицах страниц не зная. Набрать их
/// поштучно через `alloc` нельзя: свободный список отдаёт фреймы в произвольном порядке, и
/// «почти всегда подряд» здесь худший из возможных ответов — ошибка проявилась бы редко и
/// далеко от причины.
///
/// Берём ТОЛЬКО с bump-пути: он по построению отдаёт непрерывный кусок региона. Свободный
/// список для этого негоден, и склеивать его мы не пытаемся — задача «найти непрерывный кусок
/// среди возвращённых» это уже аллокатор с дефрагментацией, а он тут не нужен: кольца заводятся
/// на всю жизнь драйвера.
pub fn alloc_contig(pages: usize) -> Option<usize> {
    if pages == 0 {
        return None;
    }
    let bytes = pages.checked_mul(PAGE_SIZE)?;
    let pa = bump(bytes)?;
    unsafe { core::ptr::write_bytes(ptr(pa), 0, bytes) };
    Some(pa)
}
