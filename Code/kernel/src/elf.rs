...
//!
//! До сих пор весь код процессов был **вшит в ядро**: функции в [[user-mode|`.user`]] — общая,
//! read-only, отображённая всем процессам сразу (см. `paging::init`). Веха 19 добавляет ВТОРОЙ
//! путь — код приходит СНАРУЖИ, как обычный статический ELF64/RISC-V (собранный отдельным
//! крейтом, `programs/hello`, и хранящийся как объект [[object-model|store]] по content-id, а не
//! в образе ядра). Этот модуль — узкий и осознанно **минимальный** мост между «байтами ELF» и
//! «замапленным адресным пространством процесса»:
