...
// Веха 135.2 — было 16/8. Глубина приёмного кольца это не только «сколько кадров переживёт
// всплеск», но и ПОТОЛОК СКОРОСТИ TCP: по ней выставляется `max_burst_size` (net_phy.rs), а по
// нему smoltcp зажимает объявляемое окно. Длина кольца в байтах обязана делиться на 128 (RDLEN
// по спецификации), 64 × 16 = 1024 — делится.
const NRX: usize = 64; // дескрипторов приёма (по одному буферу-фрейму на каждый)
const NTX: usize = 32; // дескрипторов передачи
...
/// Глубина приёмного кольца (0 — карты нет). У e1000 она наша, без согласования с устройством.
...
pub fn init() -> bool {
    ...
    // По кольцу на фрейм. Раньше оба жили в одном фрейме, а TX начиналось со смещения 0x200 —
    // то есть RX упирался в TX уже на 32 дескрипторах, и рост NRX МОЛЧА затирал бы кольцо
    // передачи. Отдельные фреймы убирают связь между двумя числами и заодно дают обоим кольцам
    // выравнивание по странице (спецификация требует 16 байт — с запасом).
    let Some(rx_ring) = frame::alloc() else { return false };
    let Some(tx_ring) = frame::alloc() else { return false };
