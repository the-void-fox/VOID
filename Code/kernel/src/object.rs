...
/// Веха 101 — group commit по порогу срабатывает и в ТЕСНОМ ЦИКЛЕ, а не только когда процесс
/// уснул. До этого [`maybe_commit`] звал лишь планировщик «при возобновлении процесса», и
/// программа, льющая объекты подряд (распаковка пакета, загрузка по сети), не давала ему ни
/// одного шанса: несинхронизированное копилось в куче ЯДРА, пока та не кончалась — а кончалась
/// она ПАНИКОЙ, не отказом. Замер (`store-probe`): 8 МиБ пачкой проходили, 16 МиБ роняли машину.
///
/// Замок здесь уже отпущен: `maybe_commit` берёт его сам.
fn after_put() {
    maybe_commit();
}
...
pub fn commit_if_dirty() {
    ...
    // Веха 97.1: строка ОТЛАДОЧНАЯ и должна глушиться `log off`, как весь трейс шлюзов. Была
    // обычным `println!` — и лезла на экран после любой работы с файлами, игнорируя настройку.
    // Найдено владельцем на X54C.
