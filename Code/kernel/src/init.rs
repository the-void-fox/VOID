...
//! `power` (право выключить машину, Веха 101), `mmio:УСТРОЙСТВО`, `dma`, `sysview[:RW]` (обзор
//! процессов, Веха 153), `env` (передать ARCH/SYSTEM). Буквы прав: `r`=READ `w`=WRITE `x`=EXEC
//! `s`=SEND `g`=GRANT. Суффикс `!` у токена (Веха 154) — «право моё, детям НЕ наследуется»:
//! держатель им пользуется, спавн его не копирует (`mmio:fb!`, `power!`, `sysview:rwg!`).
...
/// Веха 173 — что доделывает ПЕРВАЯ загрузка чистого диска: сеет `/etc/system/*.vv`.
///
/// Обычная строка конфига, только приписанная к нему на один запуск. `service`, потому что это
/// программа, которая делает своё дело и уходит; права те же, что у шелла и в том же порядке
/// (Веха 99.1: `vvsh` ждёт файловый сервер первым, store вторым).
const FIRST_BOOT_SEED: &str = "service vvsh endpoint:posixfs store:rwx arg:init-config\n";
...
#
# Веха 155 — `sysview:rwg!`: право ВИДЕТЬ процессы и отзывать их права. Оно у композитора не для
# него самого: он держатель-раздатчик. Кому отдать, сказано ниже строкой `desktop sysview` —
...
default files fm
# Веха 171 — ПАНЕЛЬ И СТОЛ включены сразу. Раньше их здесь не было, и первая загрузка показывала
# пустой чёрный экран с одним окном терминала: панель, обзор столов и часы существовали, но
# включались только своим конфигом (`init-config` → `rebuild`), то есть после того, как человек
# уже решил, что смотреть тут не на что. `builtin:void` — стол, нарисованный на месте: файла не
...
fn apply_with(config: &str, known: Vec<(String, usize)>) -> Vec<(String, usize)> {
    ...
    for entry in void_conf::entries(config) {
        ...
        // Веха 152.3 — потолок наделения: наделение собрано, значит теперь можно отозвать
        // персистентные права домена (тёзка прошлой загрузки), не покрытые ИМ. Так `.cspace`
        // не даёт власть сверх конфига этого поколения (находка №1, [[redteam]]).
        proc::clamp_endowment(pid);
...
pub fn boot() {
    // Чистый диск: посеять поколения по умолчанию и выбрать то, которое машина может показать.
    //
    // Веха 171 — **по умолчанию оконный режим, если есть фреймбуфер**. Раньше первая загрузка
    // всегда садилась в текстовый `vvsh`, а рабочий стол включался вручную (`switch gen4`), — и
    // человек, впервые запустивший VOID, видел стену журнала и приглашение шелла. Композитор,
    // бар, менеджеры файлов и задач при этом были на месте; просто никто про них не говорил.
    //
    // Где фреймбуфера нет (riscv64 в QEMU, x86 без графического режима от GRUB), выбор прежний:
    // оконное поколение там не поднимется вовсе, и предложить его значило бы предложить пустоту.
    let windowed = arch::video_mode().is_some();
    let clean = object::root("system/gen1").is_none();
    if clean {
        write_text("system/gen1", DEFAULT_GEN1);
        write_text("system/gen2", DEFAULT_GEN2);
        write_text(CURRENT_ROOT, if windowed { "gen4" } else { "gen1" });
        println!("  [init] чистый диск — посеяны поколения gen1 (полное) и gen2 (без сети)");
        if windowed {
            println!("  [init] есть фреймбуфер — по умолчанию беру gen4 (оконный режим)");
        }
    }
    ...
    // Веха 173 — на ЧИСТОМ диске досеять и ТЕКСТ конфига, а не только поколения.
    //
    // Поколения (`system/gen*`) — это уже вычисленный конфиг; исходники, которые человек правит
    // (`/etc/system/*.vv`), до сих пор появлялись только по команде `init-config`. То есть свежая
    // система умела рассказать про `rebuild` (это делает окно «добро пожаловать»), но файла, к
    // которому она отсылает, на ней не было. Владельцу это стоило одного и того же обряда после
    // каждой установки: `init-config` → правка → `rebuild`.
    //
    // Досеиваем ЗАПУСКОМ `vvsh init-config`, а не копией текстов в ядре. Тексты живут в `vvsh`
    // вместе с языком, которым написаны, и вторая их копия здесь разошлась бы с первой — вопрос
    // только когда. Строка добавляется к конфигу ЭТОЙ загрузки и в store не попадает: это
    // действие первого запуска, а не часть поколения.
    let config = if clean {
        alloc::format!("{}{}", config, FIRST_BOOT_SEED)
    } else {
        config
    };
