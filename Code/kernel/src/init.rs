...
//!
//! Веха 112 — `packages имя…` читает `pkg sync` (пакеты, которые система обязана иметь).
...
//! сам). Ядро их пропускает: конфигурация системы — одна вещь с одной историей поколений, а кто
//! какие строки из неё берёт — дело читателя.
...
fn apply(config: &str) {
    ...
    for line in config.lines() {
        ...
        // Веха 112 — `packages …` тоже не ядру: объявленные пакеты собирает `pkg sync`, а ядру
        // на загрузке до них дела нет (они уже лежат в store, если собрались).
        if kind == "terminal" || kind == "bind" || kind == "packages" {
            continue;
        }
