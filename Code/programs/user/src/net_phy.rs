...
/// (`rdtime`/`rdtsc`). Переводим здесь, чтобы у стека не было ни одного представления о нашем
/// железе.
///
/// Веха 136 — цену тика спрашиваем у ядра ([`crate::ticks_to_ns`]), а не берём из константы. С
/// константой «1 нс на тик» время стека на этой машине шло ВТРОЕ быстрее настоящего: все сроки
/// TCP и DNS истекали втрое раньше объявленных, и это было видно только по дампу провода.
pub fn now() -> Instant {
    Instant::from_millis((crate::ticks_to_ns(crate::now() as u64) / 1_000_000) as i64)
}
...
impl phy::Device for VoidDevice {
    ...
    fn capabilities(&self) -> DeviceCapabilities {
        ...
        //
        // Кольца приёма в ядре углублены до 64 дескрипторов у обоих драйверов (Веха 135.2), у
        // каждого свой буфер-кадр. Берём ПОЛОВИНУ, а не все 64: кольцо должно принять не только
        // всплеск по объявленному окну, но и то, что придёт ПОКА мы его разгребаем. Завышать
        // нельзя — завышенное окно означает потерянные кадры вместо медленных.
