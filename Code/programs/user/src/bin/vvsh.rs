...
/// именем командой нельзя ([[void-notifications]]).
///
...
#\n\
# Адрес по умолчанию приходит от DHCP. Ниже — необязательные настройки; каждая пишется\n\
# отдельным токеном `arg:` в той же строке service(...):\n\
#\n\
#   \"arg:dhcp=off\", \"arg:ip=10.0.2.15/24\", \"arg:gw=10.0.2.2\"  — статика вместо DHCP\n\
#   \"arg:dns=1.1.1.1,9.9.9.9\"            — свои резолверы (до четырёх, в порядке опроса)\n\
#   \"arg:host=роутер.дом=192.168.1.1\"    — своя запись имени: отвечаем сами, не спрашивая сеть\n\
#   \"arg:block=dns/block\"                — СПИСОК БЛОКИРОВКИ (например, реклама)\n\
#\n\
# Список блокировки — обычный объект store: имена по одному в строке либо строки формата\n\
# hosts (`0.0.0.0 имя`), `#` — комментарий. Имя из списка закрывает и все его поддомены.\n\
# Кладётся загрузкой прямо из шелла и живёт вместе с поколением:\n\
#\n\
#   \\fetch(\"https://пример/hosts\", \"dns/block\")\n\
#   \\rebuild()\n\
#\n\
# Чтобы сервис МОГ прочитать список, ему нужно право на store — добавьте \"store:r\":\n\
#   [service(\"net-srv\", \"dev:net:rw\", \"store:r\", \"arg:block=dns/block\")]\n\
...
# mode = \"wm\"   — ОКОННЫЙ РЕЖИМ: композитор, окна, мышь (Вехи 117-119). ПО УМОЛЧАНИЮ.\n\
#\n\
...
# что рядом кто-то работает, и это состояние по умолчанию, а не недоделка.\n\
#\n\
# Право держит композитор и раздаёт по просьбе — но только названному ЗДЕСЬ. Кто просит, он\n\
# спрашивает у ЯДРА, а не у просителя: имя процесса равно имени программы в store, из которой\n\
# загружен образ, поэтому назваться диспетчером можно только будучи им. Уезжает право урезанным\n\
# (смотреть и отзывать — да, вручать третьим — нет).\n\
#\n\
# Веха 159 — панель просит то же право под метрики: загрузка процессора и занятая память —\n\
# такое же наблюдение за системой, как список процессов, и ambient-доступа к ним нет.\n\
# Пустой список — не отдавать никому: диспетчер тогда честно скажет в окне, что смотреть не\n\
# дано, а панель просто не покажет метрик.\n\
sysview = [\"taskmgr\", \"bar\"]\n\
\n\
...
#\n\
# \"builtin:void\" (Веха 141, переделано в 158) — стол, нарисованный на месте: файла не нужно,\n\
# любой экран получает свой точный размер. Это и стоит здесь по умолчанию. Знака VOID на столе\n\
# больше нет — он стал иконкой системы и живёт в меню оболочки.\n\
#\n\
...
bar = true\n\
\n\
# СКОРОСТЬ АНИМАЦИЙ композитора в миллисекундах (Веха 142): переезд окна, открытие, закрытие и\n\
# обзор. Это срок ПЕРЕЕЗДА, остальные считаются от него теми же долями — рассогласовать их\n\
# нельзя намеренно. 0 — без анимаций вовсе, потолок 1000.\n\
anim = 360\n\
\n\
# ВИД ОБОЛОЧКИ (Веха 144): панель, меню и родные программы рисуются тулкитом void-ui, а он\n\
# берёт цвета и размеры отсюда. Пустой список — вид по умолчанию (тот же, что у окон).\n\
#   ключи:  accent text muted panel border danger on-accent — цвет #rrggbb\n\
#           desktop band band-active — цвета стола и полос обзора (их рисует композитор)\n\
...
#   packages.vv: want = [\"nerd-fonts-fira-mono\"]   → ui(\"font\", \"FiraMonoNerdFont-Regular.otf\")\n\
# Имени не знаешь — напиши любое: панель скажет в журнал, какие файлы шрифтов вообще нашла.\n\
look = [\n\
\x20 # ui(\"font\", \"FiraMonoNerdFont-Regular.otf\"),\n\
...
# capability, — поэтому на вопрос «чьё это» отвечает не имя человека, а имя УСТРОЙСТВА. Его\n\
# показывает меню оболочки.\n\
#   name   — как зовут машину (любая строка)\n\
#   avatar — картинка (PNG/JPEG): КОРЕНЬ STORE (`f/etc/avatar.png`) ЛИБО абсолютный путь\n\
#            (`/nix/store/…/face.png`) — что удобнее, то и пишите. Круг вырезается сам.\n\
who = [\n\
\x20 # device(\"name\", \"voidbook\"),\n\
\x20 # device(\"avatar\", \"f/etc/avatar.png\"),\n\
]\n\
\n\
...
\n\
# Право обзора композитор держит всегда — как и выключение: отдать его он может только тому,\n\
# кого назвал список выше, а держать право, которое никому не отдано, безвредно.\n\
sysviewcap = \"sysview:rwg!\"\n\
\n\
# Суффикс `!` (Веха 154) — «право моё, детям НЕ наследуется». Ребёнок получает копию таблицы\n\
...
/// Веха 167 — ПРОГРАММЫ ПО УМОЛЧАНИЮ. Отдельным модулем, а не строчками в `terminal.vv`, по
/// той же причине, по какой отдельны `bar.vv` и `packages.vv`: это отдельный вопрос, и человек
/// правит его отдельно от всего прочего.
const APPS_VV: &str = "# apps.vv — ЧЕМ ОТКРЫВАТЬ: программы по умолчанию (Веха 167).\n\
#\n\
# `(default роль имя)` — роль это «чем открывать вот такое», имя это корень программы в store\n\
# (тот же, которым её запускает строка запуска). Спрашивает их любая программа, а не одна:\n\
# файловый менеджер — терминал для пункта «открыть в терминале», он же — редактор для файлов.\n\
#\n\
# Роли, которые сегодня кто-то спрашивает:\n\
#   terminal — окно с шеллом (`term`)\n\
#   files    — файловый менеджер (`fm`)\n\
#   editor   — редактор текста (`ved`; открывать им пока нечему, роль объявлена вперёд)\n\
#\n\
# Роли, которой нет в этом файле, просто нет: программа, спросившая её, честно скажет об этом,\n\
# а не подставит своё мнение молча.\n\
[\n\
\x20 default(\"terminal\", \"term\"),\n\
