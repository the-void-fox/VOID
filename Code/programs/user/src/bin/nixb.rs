...
//! ## Чего он НЕ делает
//!
//! Не разрешает зависимости: `.drv` со входами-деривациями отвергается прямо, а не собирается
//! наполовину. Порядок сборки — это граф, планировщик и разговор про параллельность; на шаге 4
//! речь про ОДНУ деривацию, и делать вид, что про граф, было бы обманом. Не проверяет хэши
//! фиксированных выходов (нечем: сеть сюда не ходит). Не ловит вывод сборки — он идёт в журнал
//! ядра, как и всякий вывод чужого процесса без stdio.
...
fn build(fs: usize, store: usize, drv_path: &[u8]) -> Result<String, String> {
    ...
    if !d.input_drvs.is_empty() {
        return Err(format!(
            "у деривации {} входов-деривациями — их сборка это граф, а не одна команда",
            d.input_drvs.len()
        ));
    }
