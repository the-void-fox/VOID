...
struct Bar {
    ...
    showing: Menu,
    /// Веха 169 — **ПЕРЕЕЗД идёт ДЕФОРМАЦИЕЙ**: какого размера полотно было на экране в момент,
    /// когда нажали на другую кнопку. `None` — никуда не переезжаем.
    ///
    /// Веха 168.2 делала это иначе: нынешнее полотно втягивалось в полосу целиком, и только
    /// потом оттуда вытягивалось новое. Движение было — но оно шло ДВА срока подряд, и владелец
    /// сказал ровно то, что видно: «слишком долго, пока меню дождётся закрытия другого».
    ///
    /// Довод в пользу деформации не только про скорость. Полотно у нас — **вещество** (см.
    /// [`Bar::sheet`]): оно не появляется и не исчезает, а вытягивается из полосы. У вещества,
    /// которому дали другое содержимое, нет причины сперва втянуться: оно меняет ФОРМУ. Поэтому
    /// содержимое подменяется сразу, а прямоугольник едет из старого размера в новый — и то, что
    /// в него не влезает, режется клипом, как режется всё в этом полотне.
    ///
    /// Хранится РАЗМЕР, а не полотно-источник: цель считается каждый кадр заново
    /// ([`Bar::card_rect`]) и может ехать сама (уходящее уведомление тянет высоту), а начало
    /// движения — величина известная и неподвижная.
    morph: Option<(i32, i32)>,
    /// Какого размера полотно НАРИСОВАНО. Отсюда переезд берёт своё начало: спрашивать
    /// [`Bar::card_rect`] в этот момент нельзя — она уже ответит про НОВОЕ меню.
    card_size: (i32, i32),
...
impl Bar {
    ...
    ///
    /// Веха 169 — переезд между полотнами буфера не трогает вовсе: полотно остаётся вытянутым и
    /// меняет форму ([`Bar::morph`]). Раньше здесь стоял ещё и «ждущий» пункт — полотно на время
    /// переезда втягивалось в полосу, и без него панель отдавала бы буфер посреди движения и тут
    /// же просила обратно.
    ...
    fn act(&mut self, surf: &Window) -> bool {
        ...
        if let Some(want) = core::mem::take(&mut self.toggle) {
            ...
            if self.open == want {
                ...
                // Веха 169 — ПЕРЕЕЗД: полотно не втягивается и не вырастает заново, оно меняет
                // форму. Начало движения — тот размер, который сейчас на экране; цель считается
                // каждый кадр сама ([`Bar::card_rect`] уже ответит про новое меню).
                if self.open != Menu::None {
                    self.morph = Some(self.card_size);
                    self.mo.set(A_MORPH, 0);
                }
                self.open = want;
                self.showing = want;
                self.fill_menu(want);
        ...
        // Форма доехала — забыть, откуда ехали. Отдельного кадра это не стоит: полотно уже
        // нарисовано своего размера, и следующий `card_rect` даст ровно то же число.
        if self.morph.is_some() && self.mo.peek(A_MORPH) >= 256 {
            self.morph = None;
        }
    ...
    fn paint(&mut self, u: &mut Ui) {
        ...
        // Веха 169 — а при ПЕРЕЕЗДЕ между полотнами прямоугольник ещё и деформируется: едет из
        // размера, который был на экране, в размер нового меню ([`Bar::morph`]). Содержимое при
        // этом уже новое и режется клипом — оно и открывается по мере того, как форма до него
        // доходит.
        let want_card = self.card_rect(th, u.font);
        let morph = self.morph;
        let card = match morph {
            None => want_card,
            Some((w0, h0)) => {
                let t = self.mo.val(A_MORPH, 256).clamp(0, 256);
                let cw = w0 + (want_card.w - w0) * t / 256;
                let ch = h0 + (want_card.h - h0) * t / 256;
                Rect::new(self.sw - cw, self.strip, cw, ch)
            }
        };
        self.card_size = (card.w, card.h);
        ...
        let card_body = sig(&[
            ...
            // Веха 169 — переезд меняет ШИРИНУ полотна, а не только его нижний край: полоска у
            // низа тут не спасает, надо перерисовывать всё. Пока никто не переезжает, число
            // стоит на месте и в подпись ничего не вносит.
            self.mo.peek(A_MORPH) as u64,
            self.confirm as u64,
            self.space as u64,
            self.spaces as u64,
            self.roots.0 as u64,
