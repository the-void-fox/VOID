...
impl App {
    ...
    fn finish_edit(&mut self, what: What, text: String) {
        ...
        match what {
            ...
            What::Rename(k) => {
                ...
                // отказ: у файлового сервера корни объектов названы ПУТЯМИ (`f/etc/x`,
                // `d/etc/x`), и переименование каталога — это перевесить корень каждого потомка
                // вглубь. Теперь персоналия это умеет (обход поддерева), и просить у неё
                // приходится ровно то же, что для файла.
    ...
    /// Веха 175 — есть ли среди выделенного НЕПУСТОЙ каталог. От этого зависит текст пункта
    /// «удалить»: снести дерево и снести пустую папку — разные поступки, и называться они
    /// обязаны по-разному.
    fn marked_nonempty_dir(&self) -> bool {
        self.marked_paths().iter().any(|p| {
            px::stat(self.ep, p.as_bytes()).is_some_and(|(d, _)| d)
                && px::readdir(self.ep, p.as_bytes(), &mut [0u8; 64]) > 0
        })
    }
    ...
    fn do_act(&mut self, a: Act, target: Tgt) {
        ...
        match a {
            ...
            Act::Delete => {
                ...
                // Веха 175 — каталог сносится ВМЕСТЕ С СОДЕРЖИМЫМ, и согласие на это уже дано:
                // пункт меню взводится первым щелчком и во взведённом виде прямо об этом
                // написано. Второго вопроса не задаём — два подряд человек перестаёт читать.
                let (mut ok, mut fail) = (0usize, 0usize);
                for p in &paths {
                    if px::unlink_all(self.ep, p.as_bytes()) == 0 {
                        ok += 1;
                    } else {
                        fail += 1;
                    }
    ...
    fn paint_menu(&mut self, u: &mut Ui, th: &Theme) -> bool {
        ...
        match target {
            Tgt::Entry(k) => {
                ...
                // щелчка, а не из сообщения после него.
