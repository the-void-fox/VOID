        ...
        match op {
            ...
            win::OP_REBUF => {
                ...
                if let Some(i) = self.wins.iter().position(|x| x.id == id) {
                    ...
                    // Веха 145 — у СЛОЯ размер буфера и есть размер поверхности: место он
                    // называет сам (то же правило, что при рождении в `OP_LAYER`). У окна ленты
                    // наоборот — там место назначает раскладка, и трогать его отсюда значило бы
                    // дать клиенту переставлять себя мимо неё.
                    //
                    // Занятую зону НЕ пересчитываем от нового размера: она названа один раз и
                    // живёт в `LayerCfg`. Иначе панель, выросшая на экран ради меню, увела бы
                    // под себя все окна.
                    if let Some(l) = self.wins[i].layer {
                        let old = self.wins[i].shown.rect();
                        let (rx, ry, rw, rh) = self.layer_rect(&l, w, h);
                        let win = &mut self.wins[i];
                        ...
                        self.damage(rx, ry, rw, rh);
                        if (rw, rh) != (w, h) {
                            let ev = [win::EV_RESIZE, rw as u8, (rw >> 8) as u8, rh as u8,
                                      (rh >> 8) as u8, 0, 0, 0];
                            self.send(i, ev, 5);
                        }
                    }
