            ...
            win::OP_ATTACH => {
                ...
                if let Some(i) = self.wins.iter().position(|w| w.id == id) {
                    ...
                    if self.wins[i].cid != cid {
                        ...
                        self.transient += need;
                        // ВРЕМЕННО 12 МиБ вместо 4 (Веха 126.3). Не оптимизация: в ядре есть
                        // ошибка на пути «уборка → надгробия → коммит», из-за которой ядро
                        // прыгает по нулевому адресу и машина встаёт намертво. Порог 4 МиБ звал
                        // уборку в разы чаще и делал ошибку достижимой за пару переездов камеры.
                        //
                        // Это НЕ лечение — ошибка остаётся и однажды сработает. Двенадцать
                        // выбраны так: заметно реже, но по-прежнему ниже кучи ядра (16 МиБ),
                        // ради чего порог и опускали с 24.
                        if self.transient > 12 * 1024 * 1024 {
                            self.transient = 0;
                            sys::obj_gc(store);
                        }
                    }
