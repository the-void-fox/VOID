...
/// Веха 155 — КОМУ композитор отдаёт своё право обзора процессов: строка `desktop sysview <имя>`
/// в конфиге поколения. Ничего нет — не отдаёт никому, и это правильный вид системы по умолчанию:
/// список процессов сам под правом, и программа без него даже не узнает, что рядом кто-то работает.
///
...
fn sysview_grantee(text: &str) -> Option<&str> {
    void_conf::get(text, "desktop", "sysview").map(str::trim).filter(|s| !s.is_empty())
}
...
fn main_loop() -> ! {
    ...
    let mut wm = Wm {
        ...
        grantee: sysview_grantee(&generation).map(String::from),
        wins: Vec::new(),
        next_id: 1,
...
struct Wm {
    ...
    sysview: usize,
    /// Веха 155 — кому отдавать обзор (`desktop sysview <имя>`); `None` — никому.
...
impl Wm {
    ...
    fn grant_for(&self, pid: usize, kind: u8) -> usize {
        // Вид 13 — Sysview. Другого композитор не раздаёт: экран и выключение он держит при себе
        // (Веха 154), и просьбу о них правильнее отклонить молча, чем обсуждать.
        if kind != 13 || self.sysview == sys::NO_CAP {
            return sys::NO_CAP;
        }
        let Some(want) = self.grantee.as_deref() else { return sys::NO_CAP };
        ...
        for k in 0..total.min(buf.len() / rec) {
            ...
            sys::write_console(
                alloc::format!(
                    "[wm] обзор процессов: P{} «{}» — {}\n",
                    pid,
                    name,
                    if ok { "выдан" } else { "ОТКАЗ (конфиг называет другого)" },
                )
                .as_bytes(),
            );
            return if ok { self.sysview } else { sys::NO_CAP };
        }
        sys::NO_CAP
    }
    ...
    fn request(&mut self, m: &sys::Message, buf: &[u8]) {
        ...
        match op {
            ...
            // Веха 155 — «дай право, которое конфиг назначил мне». Копия уезжает УРЕЗАННОЙ:
            // чтение (видеть процессы) и запись (отзывать их права) — да, `GRANT` — нет, иначе
            // диспетчер мог бы раздать обзор дальше, и объявление владельца перестало бы что-то
            // значить со второй передачи.
            win::OP_GRANT => {
