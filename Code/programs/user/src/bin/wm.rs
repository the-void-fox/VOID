...
impl Shown {
    ...
    /// Поджатое состояние — начало открытия и конец закрытия: окно поджато и ПРОЗРАЧНО.
    ///
    /// ## Почему прозрачность здесь всё-таки есть (Веха 180)
    ///
    /// Веха 179 её убрала — ради скорости: полупрозрачная строка выпадала из быстрого пути, и
    /// сборка кадра стоила 7,8 мс при бюджете 16. Скорость получили, но окно стало ИСЧЕЗАТЬ
    /// РЫВКОМ: поджалось на девять процентов и пропало. Владелец увидел это сразу.
    ///
    /// Вывод был неверный, а не замер. Правило оболочки «ничего не появляется прозрачностью»
    /// (Веха 165) — про то, что вещество ВЫТЯГИВАЕТСЯ, а не проступает из ничего; оно про
    /// появление. Исчезновение — другой вопрос: окно уходит, и уходить ему нечем, кроме как
    /// растворяясь, — поджатие на девять процентов само по себе не читается как уход.
    ///
    /// Поэтому прозрачность вернулась, а дорогой её делал не `blend`, а общий путь вокруг него:
    /// проверка углов и рамки на каждый пиксель середины. Теперь у полупрозрачной строки свой
    /// быстрый путь (см. `draw_win_row`), и проявление стоит столько же, сколько его отсутствие.
...
impl Wm {
    ...
    fn draw_win_row(
    ...
    ) {
        {
            ...
            // Веха 180 — БЫСТРАЯ СТРОКА ПОЛУПРОЗРАЧНОГО ОКНА.
            //
            // Окно исчезает и появляется ПРОЯВЛЕНИЕМ (см. `Shown::popped`), и это делает всю
            // строку полупрозрачной — то есть выбивает её из быстрого пути, который требует
            // непрозрачности. Веха 179 из-за этого прозрачность просто убрала; окно стало
            // пропадать рывком, и владелец сразу это увидел. Правильный ответ не «убрать
            // проявление», а «сделать его дешёвым».
            //
            // Середина строки при равномерной прозрачности не нуждается ни в чём из общего
            // пути: она не у угла (не нужен `rrect_sd`), не в рамке (`cover` и `inner` заведомо
            // полные), а значит весь пиксель — это «взять цвет источника и смешать с тем, что
            // под ним». Углы и края по-прежнему идут общим путём: их немного (две дуги и две
            // полоски против семисот строк середины), и считать их надо честно.
            if !opaque && alpha > 0 && has_content && !corner_row && !edge_row {
                let cs = (fx + bord).clamp(sx, ex);
                let ce = (fx + fw - bord).clamp(sx, ex);
                let off = cs - fx - bord;
                for xx in sx..cs {
                    let i = (xx - x0) as usize;
                    out[i] = self.blend(out[i], border, alpha);
                }
                if ce > cs && off >= 0 {
                    // Столбец источника — сложением, как и в непрозрачной строке (Веха 179):
                    // при проявлении окно ещё и МЕНЯЕТ РАЗМЕР, один-к-одному не бывает вовсе.
                    let cwx = cw.max(1);
                    let n0 = off * win.bw;
                    let mut col = n0 / cwx;
                    let mut err = n0 % cwx;
                    ...
                    for xx in cs..ce {
                        ...
                        if col >= 0 && col < win.bw && p + 2 < px.len() {
                            out[i] =
                                self.blend(out[i], (px[p], px[p + 1], px[p + 2]), alpha);
                        }
                        err += win.bw;
                        while err >= cwx {
                            err -= cwx;
