...
fn print_caps(sysview: usize, pid: usize) {
    ...
    if let Some(st) = sys::proc_stat(sysview, pid) {
        ...
        w("\n");
        // Веха 163 — что процесс СТОИТ машине. Время накопительное: доля процессора это разность
        // двух замеров, и считать её одной командой `ps` не из чего — она и не считается.
        w("  процессор: ");
        put_num((st.run_ns / 1_000_000) as usize);
        w(" мс всего · куча: ");
        put_num((st.heap_bytes() / 1024) as usize);
        w(" КиБ (");
        put_num(st.heap_pages as usize);
        w(" страниц)\n");
    }
}
