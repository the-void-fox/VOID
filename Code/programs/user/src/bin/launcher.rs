    ...
    fn event(&mut self, e: Event, input: &ui::Input) -> ui::Scope {
        ...
        match e {
            ...
            Event::Motion { .. } => {
                ...
                // Веха 168.2 — но ПОКА КАРТОЧКА ЕДЕТ, «часть» — это ложь, и вот какая.
                //
                // Полоса списка уже своей ширины: она вложена в карточку с полем `pad` слева и
                // справа. Клип по ней рисует карточку без этих десяти столбцов — а `self.last`
                // после кадра всё равно говорит «нарисована целиком». Следующий кадр движения
                // верит ему и трогает лишь полоску у нижнего края: столбцы по бокам так и
                // остаются от кадра, который был ДО. Вместе с ними остаётся и вырезанный тогда
                // нижний угол — на экране он висит посреди карточки ступенькой.
                //
                // Владелец это и снял: «прямоугольник разворачивается правильно, а обводка не
                // успевает». Обводка не отставала — её просто не перерисовывали.
