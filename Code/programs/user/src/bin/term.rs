...
pub extern "C" fn _start(_a0: usize, _a1: usize) -> ! {
    ...
    // Веха 167 — АРГУМЕНТ-КАТАЛОГ: в нём и открыть шелл. Нужен файловому менеджеру («открыть
    // в терминале»): терминал, открытый не там, куда человек смотрит, отвечает не на тот
    // вопрос. Сам терминал в путь не вчитывается — просто передаёт его шеллу, а тот решает
    // сам (`vvsh /путь`).
    //
    // Читается ЗДЕСЬ, до первой панели: панель заводит шелл сразу, и настройка, изменённая
    // после неё, досталась бы только второй панели — то есть никому.
    {
        let argv = sys::argv::Argv::take();
        if let Some(dir) = (0..2).filter_map(|i| argv.str(i)).find(|d| d.starts_with('/')) {
            conf.shell_args = dir.as_bytes().to_vec();
        }
    }
    ...
    loop {
        ...
        if let Out::Window { win } = &mut out {
            while let Some(ev) = win.poll_event() {
                ...
                match ev {
                    ...
                    // Веха 169 — `0xffff` это не точка, а слово композитора «курсор УШЁЛ с
                    // поверхности» (`Event::GONE`). Считать его координатой значило класть конец
                    // выделения в последнюю ячейку кадра — ровно то «выделение куда-то уезжает»,
                    // на которое жаловался владелец. С захватом указателя (Веха 169 в `wm`) уход
                    // посреди протяжки больше не приходит вовсе, но проверка остаётся: событие
                    // это законное, и молча толковать его как точку нельзя.
                    sys::win::Event::Motion { x, y } if sel_drag && x != u16::MAX => {
                    ...
                    sys::win::Event::Close => {
                        ...
                        // Веха 141 — СПЕРВА убить своих, потом уходить. Терминал — единственный
                        // вход к этим процессам: их stdio ведёт в НАШ эндпоинт, и без нас они
                        // остаются навсегда — живые, никому не видимые и ничего не могущие
                        // (`read_stdin` у них проваливается в консоль ядра, которой в оконном
                        // режиме тоже никто не даёт). Так после каждого закрытого окна в системе
                        // оставался лишний шелл. Тот же `kill`, что и при закрытии панели.
                    ...
                    // Веха 148.1 — КОЛЕСО крутит историю вывода. До него в историю можно было
                    // заглянуть только `Shift+PageUp`, и это первое, чего человек пробует мышью.
                    //
                    // Крутится панель В ФОКУСЕ, а не под курсором: мыши у терминала нет вовсе —
                    // ни выделения, ни клика по панели, — и заводить понятие «панель под
                    // указателем» ради одного колеса значило бы сделать половину мышиной
                    // поддержки и остановиться на ней.
                    //
                    // Шаг — три строки, а не полэкрана (столько у `Shift+PageUp`): колесо крутят
                    // помалу и подряд, и страница на щелчок пролетает мимо нужного места.
                    sys::win::Event::Wheel { delta, .. } => {
                        if let Some(p) = panes.get_mut(focus) {
                            ...
                            p.scroll = if delta > 0 {
                                ...
                                p.scroll.saturating_sub(step)
                            };
                            redraw = true;
                        }
                    }
                    // Веха 150.1 — в окно ЧТО-ТО УРОНИЛИ. Вставляем в фокусную панель, как
                    // будто набрали руками: перетащить имя корня в терминал — это ровно то же
                    // «понести имя в командную строку», ради которого делался и `Ctrl+C`, только
                    // без клавиатуры.
                    //
        ...
        if redraw && now.saturating_sub(last_paint) < FRAME_NS && !panes.is_empty() {
            ...
            if out.windowed() {
                    ...
                    detect_scroll(&prev_cells, &cells, view.cols, view.rows)
                {
                    ...
                    // Веха 169 — и ВЫДЕЛЕНИЕ едет вместе с текстом. Считается оно в строках
                    // кадра, а кадр только что уехал вверх: без этой поправки подсветка
                    // оставалась висеть на прежних строках, под которыми уже другие слова.
                    // Пиксели подсветки при этом уехали вместе со всем остальным (`scroll_up`
                    // двигает готовый кадр), то есть на экране она УЖЕ на новом месте — здесь
                    // за ней догоняет наша память о ней.
                    //
                    // Строка, уехавшая за верх кадра, выделение снимает целиком: держаться ему
                    // больше не за что, а половина выделения — это ложь о том, что скопируется.
                    if let Some((ac, ar, cc, cr)) = sel {
                        let moved = |r: usize| -> Option<usize> {
                            if r < start {
                                Some(r)
                            } else if r >= start + dy && r < start + dy + run {
                                Some(r - dy)
                            } else {
                                None
                            }
                        };
                        sel = match (moved(ar), moved(cr)) {
                            (Some(a), Some(c)) => Some((ac, a, cc, c)),
                            _ => None,
                        };
                    }
                }
        ...
        if !worked {
            ...
            // Веха 139.3 — срок в НАНОСЕКУНДАХ: обёртка брала тики, и «5» с «200» означали здесь
            // не миллисекунды, а такты TSC. Терминал, как и композитор, вместо сна опрашивал.
            let wait = if redraw { 5_000_000 } else { 200_000_000 };
...
///
/// Байты лежат в store, композитор назвал только их content-id: читаем своим правом на store.
/// Перевод строки НЕ пропускаем — вставить многострочный текст в шелл значит выполнить всё, кроме
/// последней строки, а этого никто не просил; заменяем пробелом, как делает большинство
/// терминалов и по той же причине.
fn paste_named(
    panes: &mut [Pane],
    focus: usize,
    read: fn(usize, &mut [u8]) -> Option<(u8, usize, usize)>,
    empty: &str,
) -> usize {
    ...
    let Some((_, got, full)) = read(cap, &mut buf) else {
        log_line(&alloc::format!("term: вставлять нечего ({})", empty));
        return 0;
    };
    for &b in &buf[..got] {
...
///
/// ## Веха 169 — и полоса обязана быть НЕ ПУСТОЙ
///
/// Восьми строк мало: пустых строк на экране терминала обычно три десятка, и пустая полоса
/// совпадает со сдвинутой копией себя при ЛЮБОМ `dy`. То есть на каждом кадре, где ничего не
/// прокручивалось, отсюда возвращался призрачный сдвиг пустоты.
///
/// Сам по себе он был безвреден — пересылка пустых строк поверх пустых ничего не меняет. Но
/// пиксели она двигает НЕ ПУСТЫЕ: поверх пустых строк лежит подсветка выделения, и её полоса
/// уезжала вверх на строку за кадр, никем не перерисованная. Это ровно то, что владелец видел:
/// «выделение постоянно куда-то уезжает».
///
/// Поэтому полоса засчитывается, только если в ней есть хоть одна строка со знаками. Пустота,
/// уехавшая вверх, — не прокрутка, а совпадение.
fn detect_scroll(prev: &[Cell], now: &[Cell], cols: usize, rows: usize) -> Option<(usize, usize, usize)> {
    ...
    let filled: Vec<bool> = (0..rows)
        .map(|y| now[y * cols..(y + 1) * cols].iter().any(|c| *c != blank))
        .collect();
    ...
    for dy in 1..=(rows / 2) {
        ...
        let real = filled[start..start + run].iter().any(|&f| f);
        if run >= MIN_BAND && real && best.map_or(true, |(_, _, b)| run > b) {
