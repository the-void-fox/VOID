...
/// Прочитать объект store целиком.
///
/// С Вехи 114 ядро называет НАСТОЯЩУЮ длину объекта вторым значением `SYS_OBJ_GET`, поэтому
/// читаем в два приёма: спросили размер маленьким буфером — выделили ровно столько — прочли.
/// До этого длина была неизвестна, «ответ ровно с буфер» и «не влезло» выглядели одинаково, и
/// буфер рос удвоением, перечитывая объект по нескольку раз.
pub fn read_object(scap: usize, id: &[u8; 32], limit: usize) -> Result<Vec<u8>, String> {
    // Маленькие объекты (а их большинство: narinfo, поколения, якорь) приезжают с первого раза.
    let mut buf = vec![0u8; 64 * 1024];
    let (n, whole) = sys::obj_get_ex(scap, id, &mut buf);
    if n == usize::MAX || (n == 0 && whole == 0) {
        return Err(String::from("объект не читается"));
    }
    if whole <= buf.len() {
        buf.truncate(whole);
    ...
    if whole > limit {
        return Err(alloc::format!(
            "объект {} Б больше потолка {} МиБ — такие приезжают блобом (кусками)",
            whole,
            limit / (1024 * 1024)
        ));
    }
    let mut buf = vec![0u8; whole];
    let (n, _) = sys::obj_get_ex(scap, id, &mut buf);
    if n != whole {
        return Err(alloc::format!("объект прочитался частично: {} из {} Б", n, whole));
    }
    Ok(buf)
}
