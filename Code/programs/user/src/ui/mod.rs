...
pub struct Ui<'a> {
    ...
    /// между собой руками.
    fade: u32,
    /// Веха 168.2 — **ЧАСЫ КАДРА**: движущиеся числа виджетов.
    ///
    /// Заводит их цикл ([`app::run`]), а не программа. До этой вехи движение было делом клиента
    /// целиком: свой [`anim::Motion`], свой `wake`, свой `tick`, — и поэтому его каждый раз
    /// приходилось ВСПОМИНАТЬ. Тумблер сети приехал в диспетчер без движения не по решению, а
    /// потому что о нём забыли; так же появлялись и остальные «дёргается вместо того, чтобы
    /// ехать».
    ///
    /// Теперь двигаться умеет сам виджет ([`Ui::anim`]), а цикл, увидев движение, сам просыпается
    /// кадром. Клиенту не нужно знать об этом ничего.
    mo: &'a mut anim::Motion,
}

impl<'a> Ui<'a> {
    pub fn new(
        px: &'a mut [u8], w: i32, h: i32, th: &'a Theme, font: &'a mut Font,
        mo: &'a mut anim::Motion,
    ) -> Ui<'a> {
        Ui {
            c: Canvas::new(px, w, h),
    ...
    /// Веха 168.2 — **число, которое САМО доезжает до цели**: `id` — имя величины, `target` — куда
    /// её ведут. Возвращается значение ЭТОГО кадра.
    ///
    /// Имя называет вызывающий, и это не педантизм: у immediate-mode тулкитов автоматические
    /// идентификаторы разъезжаются ровно тогда, когда список виджетов меняет длину, — то есть в
    /// самый неподходящий момент. Величин у одной программы десяток, и перечислить их
    /// константами дешевле, чем потом искать, почему поехал не тот тумблер.
    pub fn anim(&mut self, id: u32, target: i32) -> i32 {
        self.mo.val(id, target)
    }

    /// Двигалось ли что-нибудь на этом кадре. Спрашивает [`app::run`], чтобы решить, нужен ли
    /// следующий кадр; программе это знать не обязательно.
    pub fn moving(&self) -> bool {
        self.mo.moving()
    }

    /// Поставить величину НА МЕСТО, без движения: первый кадр не должен выглядеть как влёт.
    pub fn anim_set(&mut self, id: u32, v: i32) {
        self.mo.set(id, v);
    }
    ...
    /// Веха 166.2 — **ВСПЛЫВАЮЩЕЕ ПОЛОТНО**: панель ПОВЕРХ содержимого (контекстное меню).
    ///
    /// От [`Ui::card`] отличается не формой, а тем, на чём лежит. Карточка лежит на фоне окна и
    /// потому светлее его; всплывающее лежит НА КАРТОЧКЕ — и в её цвет сливается совершенно.
    /// Поэтому фон на ступень светлее ещё раз, а рамка не «в цвет», а видимая: у панели,
    /// накрывающей чужое содержимое, край обязан читаться, иначе непонятно, где она кончается.
    pub fn popup(&mut self, r: Rect) -> Rect {
        let bg = self.tint(self.th.band_on.with_a(0xff));
        let br = self.tint(self.th.muted.with_a(0x66));
        self.c.rrect_bordered(r, self.th.radius, self.th.line.max(1), bg, br);
        self.mark(r);
        r.inset_xy(self.th.pad, 0)
    }
    ...
    ///
    /// Веха 168.2 — **ползунок ЕДЕТ САМ**: `on` теперь цель (`true`/`false`), а не готовое число,
    /// а доезжает до неё [`Ui::anim`] под именем `id`.
    ///
    /// До этого положение считал вызывающий, и диспетчер задач честно передавал 0 или 256 — то
    /// есть тумблер прыгал. Виноват был не диспетчер: движение лежало в стороне от виджета, и
    /// про него надо было ВСПОМНИТЬ. Теперь забыть его нельзя — оно внутри.
    pub fn toggle(&mut self, r: Rect, label: &str, on: bool, hot: u32, id: u32) -> bool {
        let on = self.anim(id, if on { 256 } else { 0 }).clamp(0, 256) as u32;
