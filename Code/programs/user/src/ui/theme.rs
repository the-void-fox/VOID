...
impl Theme {
    /// Вид системы по умолчанию — палитра МАКЕТА (Веха 158).
    ///
    ...
    ///
    /// **Палитра МОНОХРОМНАЯ.** Синего акцента `#4c7dfd`, который стоял здесь до Вехи 158, в
    /// макете нет вовсе: активный стол отмечен не цветом, а СВЕТЛОТОЙ — светлая пилюля с тёмным
    /// текстом. Отсюда и `accent` серый, а `on_accent` — цвет стола.
    ///
    /// Единственный цветной здесь — `danger`. В макете опасных действий не нарисовано, а красный
    /// у кнопки выключения несёт смысл, которого светлота не несёт; когда для них появится вид,
    /// его надо будет сверить.
    pub const VOID: Theme = Theme {
        bg: Rgba::hex(0x313131),
        // Рамка В ЦВЕТ ПАНЕЛИ: в макете у островов её нет, их край держит разница со столом и с
        // полосой панели. Ключ `ui("border", …)` при этом остаётся рабочим — кому нужна рамка,
        // тот её включит, а по умолчанию системе не приписан контур, которого в макете не было.
        border: Rgba::hex(0x313131),
        text: Rgba::hex(0xdcdcdd),
        muted: Rgba::hex(0x8c8c8c),
        accent: Rgba::hex(0x9a9999),
        on_accent: Rgba::hex(0x151515),
        danger: Rgba::hex(0xf85149),
        desktop: Rgba::hex(0x151515),
        band: Rgba::hex(0x232323),
        band_on: Rgba::hex(0x3f3f3f),
        radius: 10,
        gap: 8,
        pad: 10,
        line: 1,
