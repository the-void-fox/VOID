...
impl<'a> Canvas<'a> {
    ...
    ///
    /// Веха 165 — `a` (1/256) приглушает картинку целиком. Альфа у неё СВОЯ, из пикселей, и
    /// общей прозрачностью кадра ([`super::Ui::fade`]) её было не достать: аватар оставался
    /// непрозрачным, когда всё вокруг гасло. Параметром, а не отдельным методом: забыть
    /// параметр нельзя, а забыть позвать `image_faded` — можно, и это уже случилось.
    pub fn image(&mut self, r: Rect, px: &[u8], iw: i32, ih: i32, rad: i32, a: u32) {
        ...
        for y in vis.y..vis.bottom() {
            ...
            for x in vis.x..vis.right() {
                ...
                let sa = (s[3] as u32 * a.min(256) / 256) as u8;
                self.blend(x, y, Rgba::new(s[0], s[1], s[2], sa), cov);
            }
        }
    }
