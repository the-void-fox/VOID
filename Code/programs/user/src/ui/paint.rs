    ...
    pub fn rrect_bordered(&mut self, r: Rect, rad: i32, t: i32, fill: Rgba, border: Rgba) {
        ...
        for y in vis.y..vis.bottom() {
            ...
            for x in vis.x..vis.right() {
                ...
                // Веха 169.1 — **ОДИН мазок на пиксель**, а не два, и вот почему это не мелочь.
                //
                // Раньше было так: сперва `blend(border, out − inn)`, потом `blend(fill, inn)`.
                // Каждый мазок кладётся ПОВЕРХ предыдущего, поэтому итоговое покрытие выходит не
                // `out`, а `1 − (1 − b)(1 − f)` — заметно МЕНЬШЕ. На пикселе, накрытом на 0.9, из
                // которых 0.4 приходится на заливку, получалось 0.7 вместо 0.9: пиксель темнее
                // соседей на четверть.
                //
                // Живёт это ровно там, где рамка и заливка делят пиксель, то есть на дуге угла
                // шириной в волосок рамки. Отсюда «грязь по краям скруглённых объектов»: не
                // ступенька и не лишний пиксель, а провал яркости в один пиксель вдоль всей дуги.
                // Сглаживание Вехой 169 стало точным, а провал остался — он не от точности.
                //
                // Правильно — сложить ЦВЕТ по долям, а на холст положить один раз с полным
                // покрытием: доля рамки `out − inn`, доля заливки `inn`, вес каждой умножен на её
                // альфу (иначе прозрачная рамка «затеняла» бы заливку своим весом).
                let wb = (out - inn) * border.a as u32;
                let wf = inn * fill.a as u32;
                let sum = wb + wf;
                if sum == 0 {
                    continue;
                }
                let mix = |b: u8, f: u8| ((b as u32 * wb + f as u32 * wf) / sum) as u8;
                let c = Rgba::new(
                    mix(border.r, fill.r),
                    mix(border.g, fill.g),
                    mix(border.b, fill.b),
...
///
/// ## Веха 169 — «МУРАВЬИ»: корень брался ЦЕЛЫМ, и сглаживания почти не было
///
/// Владелец описал это точно: на скруглениях есть пиксели темнее, чем должны быть, и по всей
/// системе от этого рябь. Виновата была одна строка — корень.
///
/// Считалось всё в ПОЛУПИКСЕЛЯХ, и корень возвращал целое их число. Значит расстояние до края
/// знало о себе с точностью полпикселя, а покрытие (`128 − d`) выходило кратным 128 — то есть
/// на весь край угла приходилось РОВНО ТРИ значения: 0, половина и полное. Дуга с тремя уровнями
/// яркости — это не сглаженная дуга, а лесенка с одной серой ступенькой; пиксель, накрытый на
/// четыре пятых, светился ровно наполовину.
///
/// Теперь корень берётся с точностью 1/128 полупикселя: `sqrt(v · 2¹⁴) = 128·sqrt(v)`, всё тем же
/// целочисленным корнем, только в `i64` (значение под ним — до 2¹¹, в `i32` не помещается).
/// Уровней стало 256 вместо трёх, и стоит это ровно ноль лишних операций на пиксель: корень как
/// был один, так и остался.
fn rrect_sd(px: i32, py: i32, r: Rect, rad: i32) -> i32 {
    ...
    // Длина сразу в 1/256 пикселя: полупиксели ×128.
