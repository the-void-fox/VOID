...
pub mod argv;
/// Веха 178 — язык интерфейса (`ui language en` в конфиге). Здесь, а не в тулките, потому что
/// переводить надо и тому, у кого тулкита нет: шеллу.
pub mod i18n;
...
pub struct ProcStat {
    ...
    /// Веха 163 — страниц КУЧИ (по 4 КиБ), реально занятых группой процесса. Это не «вся память
    /// процесса»: образ программы и стек отображены отдельно и сюда не входят. У ленивой кучи
    /// считается фактическое, а не запрошенное: попросить 4 МиБ и не тронуть их ничего не стоит.
    pub heap_pages: u32,
    /// Веха 163 — процессорное время процесса, нс (простой вычтен). Проценты считает тот, кто
    /// показывает: мгновенной загрузки не бывает, бывает «за промежуток» — см. [`ProcStat::cpu_percent`].
    pub run_ns: u64,
}

impl ProcStat {
    /// Байт кучи (страницы × 4 КиБ).
    pub fn heap_bytes(&self) -> u64 {
        self.heap_pages as u64 * 4096
    }

    /// Доля процессора за промежуток между двумя замерами, в процентах. `wall_ns` — сколько
    /// прошло по часам за тот же промежуток. Ноль промежутка — ноль ответа: делить не на что,
    /// а придумывать число вместо этого нельзя.
    pub fn cpu_percent(&self, prev: &ProcStat, wall_ns: u64) -> u32 {
        if wall_ns == 0 {
            return 0;
        }
        let d = self.run_ns.saturating_sub(prev.run_ns);
        ((d.saturating_mul(100) / wall_ns) as u32).min(100)
    }
}
...
pub fn proc_stat(sysview_cap: usize, pid: usize) -> Option<ProcStat> {
    ...
    Some(ProcStat {
        ...
        heap_pages: u32::from_le_bytes([b[36], b[37], b[38], b[39]]),
        run_ns: g(40),
    })
}
...
pub fn cap_info(cap: usize) -> Option<(u8, u32)> {
    cap_info_ex(cap).map(|(k, r, _)| (k, r))
}

/// Веха 166 — то же плюс АДРЕСАТ: `(вид, права, aux)`, где `aux` — процесс на том конце
/// эндпоинта (`0xFFFF` — вид не про связь).
///
/// Нужен тому, у кого эндпоинтов несколько, — а их несколько у каждого окна (композитор,
/// файловый сервер, сервер сети). Без адресата «вид 4» не отвечает на вопрос «а это канал КУДА»,
/// и выбрать из своих каналов нужный нельзя было вовсе.
pub fn cap_info_ex(cap: usize) -> Option<(u8, u32, u16)> {
    let r = abi::syscall(SYS_CAP_INFO, cap, 0, 0, 0, 0, 0, 0).0;
    (r != NO_CAP).then(|| ((r >> 16) as u8, (r & 0xffff) as u32, (r >> 32) as u16))
}
...
pub mod posix {
    ...
    /// Веха 175 — `unlink -r`: снять каталог ВМЕСТЕ С СОДЕРЖИМЫМ. `0` — снят целиком.
    ///
    /// Отдельная функция, а не флаг у [`unlink`], намеренно: у вызова, который может потерять
    /// чужую работу, должно быть собственное имя. Спросить согласие человека обязан тот, кто с
    /// ним разговаривает, — персоналия его не спрашивает и спрашивать не умеет.
    pub fn unlink_all(ep: usize, path: &[u8]) -> usize {
        let mut r = [0u8; 1];
        crate::call(ep, OP_UNLINK | (1 << 16), path, &mut r);
        if r[0] == 0 { 0 } else { usize::MAX }
    }
