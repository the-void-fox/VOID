...
impl LzmaParams {
    // Апстримные `LzmaParams::new`, `LzmaDecoder::reset` и `process_stream` убраны вместе с
    // фичами `raw_decoder`/`stream`: копия только распаковывает готовый .xz.
...
pub(crate) struct DecoderState {
    ...
    partial_input_buf: crate::io::Cursor<[u8; MAX_REQUIRED_INPUT]>,
...
impl DecoderState {
    pub fn new(lzma_props: LzmaProperties, unpacked_size: Option<u64>) -> Self {
        lzma_props.validate();
        DecoderState {
            partial_input_buf: crate::io::Cursor::new([0; MAX_REQUIRED_INPUT]),
            lzma_props,
            unpacked_size,
    ...
    fn try_process_next<W: io::Write, LZB: LzBuffer<W>>(
        ...
        let mut temp = crate::io::Cursor::new(buf);
