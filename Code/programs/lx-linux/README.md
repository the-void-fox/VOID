...
- **Веха 69** — **портированный e1000 на НАСТОЯЩЕМ QEMU-e1000 через MMIO-cap** (`lx-e1000-hw`):
  ...
  e1000 нет → собирается/импортируется, не спавнится (как C-драйвер Вехи 54).
- **Веха 70** — **портированный e1000 ПЕРЕДАЁТ кадр (DMA-TX) на реальном QEMU-e1000** (`lx-e1000-hw`):
  `dma_alloc_coherent` в `lx_net.c` сведён с DMA-cap (`vsys_dma_alloc`, `start_cap 1`; guard
  `-DLX_HAVE_SYSCALL`); драйвер строит TX-кольцо дескрипторов vendored `e1000_setup_all_tx_resources`
...
- **Веха 131** — **atl1c (Atheros AR8151) СОБИРАЕТСЯ** против тех же шимов: неизменённые
  `atl1c_main.c` (2887 стр) + `atl1c_hw.c` (876) из 6.18.7, 0 ошибок / 0 предупреждений на обеих
  арх (`lx-atl1c-probe` — деривация-РАЗВЕДКА: компиляция без линковки, цель не программа, а
  список нехваток). Карта опознана на живом X54C Вехой 130: `1969:1083` = AR8151 v2.0, шина 04
  за мостом PCIe. Поверхность шимов, выращенная под e1000, покрыла почти всё — не хватало ОДНОГО
  заголовка (`crc32.h`). Добавлено: `linux/crc32.h` + `crc32_le`, `linux/random.h` +
  `get_random_bytes` поверх `SYS_RANDOM` (и настоящий `eth_random_addr` вместо заглушки, дававшей
  всем `02:02:02:02:02:02`), константы MII/ethtool, PCI-обвязка (AER, `pci_request_regions`,
  MSI-отказ, `pcie_*`), **очередь передачи объектом** (`netdev_queue`, `netif_tx_*`,
  `alloc_etherdev_mq`), `module_pci_driver`, `dev_err_probe`, `roundup`.

  Попутно выправлены две ошибки в старых шимах: биты `NETIF_MSG_RX_STATUS`/`TX_DONE` стояли
  МЕСТАМИ НАОБОРОТ, а `__netdev_tx_sent_queue` вернул бы `void` — драйвер не дёрнул бы TX-регистр
  ни разу, кольцо копилось бы, а карта молчала.
...
- **`lx_net.c`** — **сетевой рантайм/заглушки** (Веха 68, «generated_dummies»): ТЕЛА netdev/skb/dma/napi/
  ...
  часть — no-op под пути (open/RX/ISR), что оживут дальше. **`dma_alloc_coherent`** (Веха 70) под
  `-DLX_HAVE_SYSCALL` идёт по **DMA-cap** (`vsys_dma_alloc` → device-доступная страница), иначе (сборка-
  «вычислялка») — куча. Нужен для ЛИНКОВКИ и реального DMA драйвера.
...
**`lx-e1000-hw`** (Вехи 69–73) НЕ запускают из vsh — его спавнит init как userspace-драйвер, когда в QEMU
...
**принимает по ПРЕРЫВАНИЮ** (Веха 72: `request_irq`↔`vsys_irq_wait` по IRQ-cap start_cap 2 — планировщик
Lx_kit спит на прерывании, ISR читает ICR и будит задачу; не опрос DD), и делает **`PING` шлюза**
(Веха 73: ARP узнаёт MAC шлюза → ICMP echo request → приём `ICMP ECHO REPLY … ttl=255` — полный IP
round-trip через неизменённый Linux-драйвер; **способность хостить Linux-драйверы доказана**).
