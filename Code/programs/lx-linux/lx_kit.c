...
int printk(const char *fmt, ...)
{
	...
	/* Веха 133.1 — printk обязан быть НЕБУФЕРИЗОВАННЫМ. `printf` newlib копит вывод и отдаёт его
	 * при заполнении буфера или на выходе из программы. Харнессы это скрывали: они завершались,
	 * и буфер сбрасывался целиком. Полный драйвер из планировщика не возвращается — и весь его
	 * рассказ о том, где он застрял, оставался в буфере. То есть диагностика работала везде,
	 * КРОМЕ единственного случая, ради которого она есть: когда программа не дошла до конца. */
	if (!unbuffered) {
		unbuffered = 1;
...
void lx_sched_run(void)
{
	...
	for (;;) {
		...
		/* Готовых задач нет: ждём ближайшего срока таймера и идём на новый круг (там он
		 * выстрелит и разблокирует задачу).
		 *
		 * Веха 134 — ЖДЁМ СНОМ, а не кручением. Здесь стоял глухой цикл по монотонному
		 * времени, и это ничего не стоило, пока драйверы жили секунды: bring-up отработал и
		 * вышел. Долгоживущий драйвер (вещатель журнала спит по 200 мс в вечном цикле) на том
		 * же коде забирал бы свою долю процессора НАВСЕГДА — ровно та же ошибка, что сегодня
		 * нашлась в net-srv и объяснила «вялость первых секунд».
		 *
		 * Правило простое: «ждём события» и «крутим процессор, пока не дождёмся» — разные
		 * вещи, и вторая маскируется под первую везде, где ждать обычно недолго. */
		if (lx_timers_next(&next_exp)) {
			...
			do
				lx_jiffies_update();
			while (time_before(jiffies, next_exp)); /* без syscall'ов — только кручением */
#endif
			continue;
		}
#ifdef LX_HAVE_SYSCALL
